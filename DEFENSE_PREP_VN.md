...
| 2 | Incremental ≈ 4.4%; "hạn chế quan trọng nhất, không nên che giấu" | `04:326`, `05:36,86`, bảng | ✅ | Đúng và trung thực. Mã nguồn cho thấy lý do: `IsIncremental` cần `LTS>0.5`; trên ramp nhiễu LTS≈0.42 nên rơi xuống nhánh Gradual (`se_cdt.py:661-698`). Đây là lỗi cấu trúc, không phải bug. |
...
| 7 | Phân loại đo ở **Oracle mode** (cho sẵn vị trí drift ground-truth) | `04:264-266` | ✅ | Trung thực và sạch về phương pháp — nhưng xem câu H6: nó làm con số 80.1/50.5 lạc quan hơn so với end-to-end thật. |
...
**B. ⚠️ Phân loại chỉ ~50% đúng, nhưng Type-Specific adaptation thắng +20pp — tại sao? (bẫy sắc nhất).** Các kịch bản adaptation (Stepping/Sudden/Mixed) đều thiên về Sudden, nơi classifier mạnh (Sudden 82.4%). Bạn **chưa** thử adaptation trên luồng nặng Gradual/Incremental (nơi classifier rớt còn 4.4%). Phần kết luận cũng liệt kê "bổ sung kịch bản Gradual/Incremental" là future work (`05:65`). Vậy lợi ích adaptation chỉ được chứng minh *trong vùng mà classification hoạt động tốt*. Luyện câu trả lời H2 — đừng khẳng định type-specific adaptation đã được kiểm chứng cho mọi loại drift.
...
- **H2. Classifier chỉ ~50% đúng, vậy mà type-specific adaptation thắng baseline ~20pp. Sao một classifier như tung đồng xu lại giúp nhiều thế?** Hai lý do, nói thẳng: (1) các kịch bản adaptation (Stepping/Sudden/Mixed) thiên về Sudden, nơi classifier mạnh (Sudden 82.4%, CAT 80.1%) — định tuyến đúng phần lớn *trong vùng này*; (2) ngay cả định tuyến không hoàn hảo vẫn hơn No-Adaptation vì *bất kỳ* reset nào trên một sudden drift thật cũng có ích. Em **chưa** thử adaptation trên luồng nặng Gradual/Incremental, nơi classification rớt còn 4.4% — đó là future work (`05:65`). Vậy em chỉ khẳng định lợi ích adaptation *cho vùng sudden/recurrent*, không phải phổ quát.
...
- **H5. Incremental 4.4% gần như bằng 0 — classifier có còn hoạt động trên lớp đó không?** Thật lòng, với Incremental ở mức *tiểu loại* thì gần như thất bại: ramp chậm tạo gò rộng thấp, LTS (~0.42) trượt khỏi cổng 0.5 và rơi xuống Gradual. Nhưng *nhóm lớn* (PCD) vẫn thường đúng, và PCD→fine-tune là chiến lược thích ứng đúng dù sao, nên chi phí vận hành của nhầm lẫn Gradual/Incremental là nhỏ. Hướng sửa nằm ở future work: cửa sổ đa thang đo hoặc một đầu phân loại xác suất học trên σ(t).
- **H6. Con số 80.1%/50.5% là "Oracle mode" — cho sẵn vị trí drift thật. End-to-end thật là bao nhiêu?** Thấp hơn, và em nói rõ điều đó (`04:264`). Oracle mode cố ý tách kỹ năng *phân loại* khỏi kỹ năng *phát hiện* — đây là thông lệ chuẩn. End-to-end, các phát hiện sót/lệch vị trí (EDR 92.6% Std) sẽ kéo độ chính xác tiểu loại xuống nữa. Em báo Oracle để đo được trần năng lực của riêng classifier; em không trình bày nó như con số triển khai thực.
...
- **H8. Gamma null giả định mẫu i.i.d.; luồng thì tương quan thời gian. Kiểm định sai cỡ nào?** Trên Gaussian i.i.d., Type-I ≈ 0.04–0.05 (đúng). Trên AR(1) ρ=0.6, IDW tăng lên ~0.075 và SE-CDT composite lên ~0.100 — ~2× danh nghĩa. Em báo điều này ở `tab:h0_calibration` và diễn giải H0 như một *kiểm tra calibration thực nghiệm, không phải bảo đảm lý thuyết* cho toàn pipeline. Muốn kiểm soát chính xác thì permute (chậm 7×) hoặc block/dependent bootstrap — hướng tương lai đã nêu.
...
| CAT / SUB micro / SUB macro | 80.1% / 50.5% / 50.0% | `table_se_cdt_performance_by_type.tex` |
| Per-subtype (Sudden/Blip/Recurrent/Gradual/Incremental) | 82.4 / 60.8 / 71.5 / 30.8 / 4.4 % | cùng bảng |
| SE-CDT Std vs IDW (EDR / FP tổng) | 92.6%/4846 vs 56.7%/657 | `table_se_cdt_aggregate.tex` |
| Type-I error (iid d5/d10/AR1) — IDW / composite | 0.040–0.075 / 0.045–0.100 | `table_h0_calibration.tex` |
