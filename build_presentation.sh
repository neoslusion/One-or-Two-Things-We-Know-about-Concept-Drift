...
# Presentation PDF Build Script
# This script compiles the Beamer LaTeX presentation
...
clean_aux_files() {
    ...
    rm -f presentation.aux presentation.bbl presentation.blg presentation.fdb_latexmk \
          presentation.fls presentation.log presentation.out presentation.synctex.gz \
          presentation.toc presentation.nav presentation.snm presentation.vrb

    print_success "Auxiliary files cleaned."
    cd - > /dev/null
}

# Function to compile LaTeX presentation
compile_latex() {
    print_status "Starting Beamer presentation compilation..."
    cd "$LATEX_DIR"
    ...
    if [ $? -ne 0 ] && ! grep -q "Output written on $MAIN_FILE.pdf" pdflatex_1st.log; then
        print_error "First pdflatex pass failed. Check presentation.log for details."
        cat presentation.log | tail -20
        cd - > /dev/null
        exit 1
    fi

    # Run bibtex if .bib file exists (optional for presentations)
    if [ -f "references.bib" ]; then
        print_status "Running bibtex..."
        if ! bibtex "$MAIN_FILE" > /dev/null 2>&1; then
            print_warning "BibTeX failed, but continuing (bibliographies optional in presentations)..."
        else
    ...
    if [ $? -ne 0 ] && ! grep -q "Output written on $MAIN_FILE.pdf" pdflatex_2nd.log; then
        print_error "Second pdflatex pass failed. Check presentation.log for details."
        cat presentation.log | tail -20
        cd - > /dev/null
        exit 1
    fi

    # Third pass (to resolve all cross-references and navigation)
    ...
    if [ $? -ne 0 ] && ! grep -q "Output written on $MAIN_FILE.pdf" pdflatex_3rd.log; then
        print_error "Third pdflatex pass failed. Check presentation.log for details."
        cat presentation.log | tail -20
        cd - > /dev/null
        exit 1
    fi
    ...
    cd - > /dev/null
    print_success "Beamer presentation compilation completed successfully."
}
...
check_output() {
    if [ -f "$LATEX_DIR/$MAIN_FILE.pdf" ]; then
        ...
        print_success "PDF generated successfully: $LATEX_DIR/$MAIN_FILE.pdf (${file_size_kb}KB)"
...
show_stats() {
    ...
    if [ -f "$MAIN_FILE.log" ]; then
        local pages=$(grep -o "Output written on $MAIN_FILE.pdf ([0-9]* slides" "$MAIN_FILE.log" | grep -o "[0-9]*" | head -1)
        if [ -z "$pages" ]; then
            pages=$(grep -o "Output written on $MAIN_FILE.pdf ([0-9]* pages" "$MAIN_FILE.log" | grep -o "[0-9]*" | head -1)
        fi
        ...
        echo "  Slides: ${pages:-Unknown}"
        echo "  Warnings: $warnings"
        echo "  Errors: $errors"

        if [ "$warnings" -gt 0 ]; then
            print_warning "There were $warnings warnings. Check presentation.log for details."
        fi
    fi
    cd - > /dev/null
}
...
show_help() {
    echo "Presentation PDF Build Script"
    echo ""
...
fast_build() {
    ...
    if ! pdflatex -interaction=nonstopmode "$MAIN_FILE.tex" > /dev/null 2>&1; then
        print_error "pdflatex failed. Check presentation.log for details."
        cat presentation.log | tail -20
        cd - > /dev/null
        exit 1
    fi
...
watch_mode() {
    ...
    print_status "Watching for changes in $LATEX_DIR/presentation.tex..."
    ...
    if ! command_exists inotifywait; then
        ...
        while true; do
            sleep 2
            if [ "$LATEX_DIR/presentation.tex" -nt "$LATEX_DIR/$MAIN_FILE.pdf" 2>/dev/null ]; then
        ...
        # inotify mode
        while true; do
            inotifywait -e modify "$LATEX_DIR/presentation.tex" -q
            print_status "Changes detected, rebuilding..."
