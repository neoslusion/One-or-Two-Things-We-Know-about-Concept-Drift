...
\begin{frame}{Cải tiến 2: MMDAgg - Aggregated Maximum Mean Discrepancy}
\textbf{Phương pháp:} ShapeDD\_MMDAgg (JMLR 2023)

\vspace{8pt}
\textbf{Ý tưởng chính:}
\begin{itemize}
  \item \alert{Insight:} Kết hợp nhiều kernel bandwidth để robust hơn!
  \item Thay vì dùng 1 bandwidth cố định → Aggregate nhiều bandwidths
  \item Tận dụng MMDAgg từ Gretton et al. (JMLR 2012)
\end{itemize}

\vspace{8pt}
\textbf{Quy trình hoạt động:}
\begin{enumerate}
  \item \textbf{Multiple Kernels:} Sử dụng nhiều Gaussian kernels với các bandwidth khác nhau
  \begin{itemize}
    \item Small bandwidth: bắt local changes
    \item Large bandwidth: bắt global changes
  \end{itemize}

  \item \textbf{Aggregation:} Kết hợp các MMD statistics
  \begin{itemize}
    \item Weighted combination theo variance
    \item Tự động cân bằng contribution của từng kernel
  \end{itemize}

  \item \textbf{Kết quả:} Robust hơn với nhiều loại drift patterns
\end{enumerate}
\end{frame}
...
\begin{table}
  ...
  \begin{tabular}{lcccc}
    ...
    \midrule
    ShapeDD (gốc) & 0.592 & Baseline, single kernel & So sánh baseline \\
    \midrule
    \rowcolor{yellow!20}
    \textbf{ShapeDD\_MMDAgg} & \textbf{0.167} & \textbf{Multi-kernel aggregation} & \textbf{MAIN - Robust} \\
    \midrule
    ShapeDD\_OW\_MMD & 0.60-0.62 & Optimally-weighted MMD & High precision needed \\
    \midrule
    MMD\_OW & 0.62-0.65 & Pure OW-MMD & Direct comparison \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{center}
  ShapeDD (baseline) → Shape Analysis \textbf{+} MMD \\
  → \alert{ShapeDD\_MMDAgg (multi-kernel)} ← \textbf{MAIN} \\
  ↙ \hspace{3cm} ↘ \\
  ShapeDD\_OW\_MMD \hspace{2cm} MMD\_OW
\end{center}

\vspace{8pt}
\textbf{Contribution chính:} Kết hợp shape analysis với aggregated MMD để robust với nhiều drift patterns
\end{frame}
