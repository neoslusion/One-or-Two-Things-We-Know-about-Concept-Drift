...
\author[Nguyen Duc Trung]{%
  Sinh viên thực hiện: Nguyen Duc Trung\\[0.5em]
  \small Giảng viên hướng dẫn: TS. Nguyễn An Khương
}
...
\begin{frame}{Thiết lập thực nghiệm Benchmark}
    ...
    \footnotesize{\textit{*ShapeDD\_WMMD đạt cân bằng tốt nhất giữa tốc độ và độ tin cậy (0 False Alarm cho P(X) changes).}}
\end{frame}
    ...
    \begin{columns}[T]
        \begin{column}{0.5\textwidth}
            \begin{table}
                ...
                \begin{tabular}{lc}
                    \toprule
                    \textbf{Method} & \textbf{Accuracy (Category)} \\
                    \midrule
                    \textbf{SE-CDT (Proposed)} & \textbf{85.8\%} \\
                    CDT\_MSW (Supervised) & 38.7\% \\
                    \bottomrule
                \end{tabular}
            \end{table}
        \end{column}
        \begin{column}{0.5\textwidth}
            \textbf{Phân tích:}
            \begin{itemize}
                \item SE-CDT hoạt động ổn định nhờ tín hiệu ShapeDD rõ ràng.
                \item CDT\_MSW bị giới hạn bởi độ trễ của classifier trong cửa sổ nhỏ.
                \item \textit{Hạn chế:} Độ chính xác phân loại chi tiết (Sub-category) còn thấp ở các lớp Gradual/Incremental.
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}
...
\begin{frame}{Hạn chế & Thách thức}
	\begin{alertblock}{Độ nhạy với Virtual Drift}
		Phương pháp hiện tại \textbf{không phát hiện} Virtual Drift (chỉ thay đổi $P(Y|X)$).
		\begin{itemize}
			\item \textit{Lý do:} ShapeDD hoạt động dựa trên $P(X)$ (unsupervised).
			\item \textit{Góc độ tích cực:} Giúp hệ thống ổn định, không báo động giả khi phân phối đầu vào vẫn giữ nguyên (tính chất bảo toàn $P(X)$).
		\end{itemize}
	\end{alertblock}

	\vspace{1em}
	\textbf{Các hạn chế khác:}
	\begin{itemize}
		\item Độ chính xác phân loại các drift phức tạp (Gradual/Incremental) chưa cao.
		\item Chưa đánh giá sâu khả năng chịu lỗi (fault-tolerance) trong môi trường phân tán thực tế.
	\end{itemize}
\end{frame}

\begin{frame}{Hướng phát triển tương lai}
	\begin{itemize}
		\item \textbf{Ưu tiên 1:} Cải thiện độ nhạy với Gradual Drift (nghiên cứu đặc trưng Shape curve phẳng).
		\item \textbf{Ưu tiên 2:} Tích hợp Meta-Learning để tự động chọn chiến lược thích ứng.
		\item \textbf{Dài hạn:} Xây dựng bộ Benchmark chuẩn với dữ liệu thực tế (Real-world datasets) từ các đối tác công nghiệp.
	\end{itemize}
\end{frame}
