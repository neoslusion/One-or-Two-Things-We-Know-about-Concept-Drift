...
\begin{frame}{ShapeDD - Ưu và nhược điểm}
  ...
  \begin{block}{Động lực cải tiến}
    Các hạn chế này dẫn đến các cải tiến: MMDAgg (multi-kernel), OW-MMD (optimal weighting)
  \end{block}
\end{frame}
...
\begin{frame}{Cải tiến 1: Optimally-Weighted MMD (OW-MMD)}
\textbf{Vấn đề:}
\begin{itemize}
  \item MMD truyền thống dùng trọng số đều ($1/n$), coi mọi mẫu dữ liệu quan trọng như nhau.
  \item Phương sai bộ ước lượng cao $\to$ cần nhiều mẫu hoặc bootstrap lớn (2500 lần) để ổn định.
\end{itemize}

\vspace{8pt}
\textbf{Giải pháp: Trọng số tối ưu giảm phương sai}
\begin{itemize}
  \item Gán trọng số nghịch biến với mật độ dữ liệu:
  \[ w_i \propto \frac{1}{\sqrt{\sum_j k(x_i, x_j)}} \]
  \item Giảm trọng số các vùng dữ liệu dày đặc (thông tin dư thừa), tăng trọng số vùng biên (nhạy cảm với drift).
\end{itemize}

\vspace{8pt}
\textbf{Hiệu quả đột phá:}
\begin{itemize}
  \item \alert{Tăng tốc độ > 7 lần:} Giảm số lần bootstrap từ 2500 xuống 10-50 lần.
  \item \alert{Thông lượng:} $\sim$ 50,000 samples/s (so với 6,400 của ShapeDD gốc).
\end{itemize}
\end{frame}

\begin{frame}{Cải tiến 2: MMD Aggregated (MMD-Agg)}
\textbf{Vấn đề:}
\begin{itemize}
  \item Kernel RBF đơn lẻ với độ rộng $\sigma$ cố định chỉ bắt được một loại drift nhất định.
  \item $\sigma$ nhỏ: Bắt nhiễu cục bộ (Blip) nhưng bỏ lỡ dịch chuyển toàn cục.
  \item $\sigma$ lớn: Bắt dịch chuyển lớn nhưng làm mờ các thay đổi tinh vi.
\end{itemize}

\vspace{8pt}
\textbf{Giải pháp: Thích nghi đa hạt nhân}
\begin{itemize}
  \item Quét trên tập hợp nhiều bandwidths: $\mathcal{K} = \{k_{\sigma_1}, k_{\sigma_2}, \dots, k_{\sigma_M}\}$.
  \item Thống kê kiểm định tổng hợp:
  \[ \Delta_{Agg} = \max_{k \in \mathcal{K}} (\widehat{\text{MMD}}_k^2 - \lambda_k) \]
\end{itemize}

\vspace{8pt}
\textbf{Hiệu quả:}
\begin{itemize}
  \item \alert{Tối ưu Minimax:} Đảm bảo phát hiện mọi loại thay đổi phân phối (local & global).
  \item \alert{F1 = 1.0 trên RBF Blips:} Vượt trội hoàn toàn so với các phương pháp đơn kernel.
\end{itemize}
\end{frame}

\begin{frame}{Kết quả Benchmark Tổng hợp (30 Runs)}
\begin{table}
  \centering
  \small
  \begin{tabular}{clcccc}
    \toprule
    \textbf{Rank} & \textbf{Method} & \textbf{F1-Score} & \textbf{Precision} & \textbf{Recall} & \textbf{Speed (s/s)} \\
    \midrule
    \rowcolor{green!10}
    1 & \textbf{MMD\_OW} & \textbf{0.576} & \textbf{0.627} & 0.578 & \textbf{51,282} \\
    \rowcolor{green!10}
    2 & \textbf{ShapeDD\_OW\_MMD} & 0.562 & 0.620 & 0.555 & \textbf{52,549} \\
    3 & D3 & 0.517 & 0.590 & 0.500 & 27,693 \\
    4 & ShapeDD\_MMDAgg & 0.509 & 0.562 & 0.504 & 6,111 \\
    5 & MMD & 0.475 & 0.368 & 0.713 & 6,449 \\
    6 & DAWIDD & 0.460 & 0.352 & 0.710 & 3,253 \\
    7 & ShapeDD (Gốc) & 0.440 & 0.322 & 0.731 & 6,399 \\
    8 & KS Test & 0.299 & 0.197 & \textbf{0.775} & 28,161 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\textbf{Nhận xét:}
\begin{itemize}
  \item \textbf{OW-MMD:} Vô địch về F1 và Tốc độ (nhanh nhất, chính xác nhất).
  \item \textbf{KS Test:} Recall cao nhất nhưng quá nhiều báo động giả (Precision thấp nhất).
\end{itemize}
\end{frame}
...
\begin{frame}{Thiết kế thực nghiệm (Cập nhật)}
  ...
  \begin{itemize}
    ...
    \item Permutations: 2,500 (cho ShapeDD gốc) vs Optimized Weights (cho OW-MMD)
  \end{itemize}
\end{frame}
...
\begin{frame}{SOTA Methods (2024-2025)}
  \begin{table}
    ...
    \begin{tabular}{llccc}
      ...
      \rowcolor{green!10}
      \textbf{Our Method (OW-MMD)} & Unsupervised & 0.576 & 0\% & 2025 \\
...
\begin{frame}{Kiến trúc hiện tại của chúng ta}
  \begin{figure}
    ...
    \begin{tikzpicture}[
      ...
      \node[box, fill=orange!10] (detector) at (5,1.5) {Component 2:\\ShapeDD MMDAgg\\(Monitors X)};
...
\begin{frame}[noframenumbering]{Backup: Confusion Matrix (Ví dụ thực tế)}
  \textbf{Ví dụ:} Kết quả của \textbf{MMD\_OW} trên bộ dữ liệu \textbf{Stagger} (10 drift points).
  ...
  \begin{table}
    ...
    \begin{tabular}{cc|cc}
      ...
      & & Drift & No Drift \\
      \hline
      \multirow{2}{*}{\textbf{Actual}} & Drift & \textbf{TP = 10} & \textbf{FN = 0} \\
      & No Drift & \textbf{FP = 0} & TN = N/A \\
    \end{tabular}
  \end{table}

  \vspace{1em}
  \begin{align*}
    \text{Precision} &= \frac{10}{10+0} = 1.00 \\
    \text{Recall} &= \frac{10}{10+0} = 1.00 \\
    \text{F1-Score} &= 1.00
  \end{align*}
\end{frame}

\begin{frame}[noframenumbering]{Backup: Runtime Comparison (Cập nhật)}
  \begin{table}
    \centering
    \small
    \begin{tabular}{lcc}
      ...
      \midrule
      \textbf{ShapeDD\_OW\_MMD} & \textbf{0.19} & \textbf{52,549} \\
      MMD\_OW & 0.20 & 51,282 \\
      KS & 0.36 & 28,158 \\
      D3 & 0.36 & 27,693 \\
      ShapeDD (Gốc) & 1.56 & 6,399 \\
      DAWIDD & 3.07 & 3,253 \\
      \bottomrule
    \end{tabular}
  \end{table}
  ...
  \vspace{0.5em}
  \textbf{Nhận xét:} OW-MMD và ShapeDD\_OW\_MMD nhanh hơn gấp \textbf{>7 lần} so với ShapeDD gốc và thậm chí vượt qua cả KS test (phương pháp thống kê đơn giản).
\end{frame}
