...
\begin{frame}{Cải tiến 2: Phân loại SE-CDT (Unsupervised)}
    ...
    \vspace{0.5em}
    \textbf{Ưu điểm:} Không cần nhãn thật để tính Accuracy Ratio như phương pháp cũ (CDT-MSW).
\end{frame}
...
\begin{frame}{Thiết lập thực nghiệm Benchmark}
    ...
    \begin{table}
        ...
        \begin{tabular}{lcccc}
            ...
            \midrule
            \rowcolor{green!10}
            \textbf{ShapeDD-IDW} & \textbf{0.481} & \textbf{0} & \textbf{18} & \textbf{131,280} \\
    ...
    \footnotesize{\textit{*ShapeDD-IDW đạt FP=0 do chỉ phát hiện thay đổi P(X), không báo động sai trên virtual drift (P(Y|X) đổi).}}
\end{frame}
...
\begin{frame}{Đánh giá Phân loại Drift (SE-CDT)}
    ...
    \textbf{Baseline:} CDT-MSW (Supervised - Cần nhãn thật).
...
\begin{frame}{Kiến trúc tổng thể}
	\begin{figure}
		...
		\begin{tikzpicture}[
			...
			\node[block, right of=detect, xshift=2cm] (classify) {CDT-MSW\\Drift Classification};
	...
	\begin{enumerate}
		...
		\item Khi phát hiện drift → Kích hoạt phân loại (CDT-MSW)
...
\begin{frame}{Thuật toán ShapeDD}
	...
	\begin{block}{Cải tiến trong nghiên cứu}
		\textbf{ShapeDD-IDW:} Tích hợp trọng số tối ưu giúp giảm phương sai và tăng tốc độ.
	\end{block}
\end{frame}

\begin{frame}{Drift Classification (CDT-MSW)}
...
\begin{frame}{Thiết kế thực nghiệm (Cập nhật)}
	...
	\begin{itemize}
		...
		\item Permutations: 2,500 (cho ShapeDD gốc) vs Optimized Weights (cho IDW-MMD)
	\end{itemize}
\end{frame}
	...
	\begin{enumerate}
		...
		\item \textbf{Về Thực nghiệm (Empirical):}
			\begin{itemize}
				...
				\item \textbf{Phân loại:} Độ chính xác SE-CDT (85.8\%) cao hơn baseline CDT-MSW (38.7\%).
			\end{itemize}
...
\begin{frame}[noframenumbering]{Backup: Runtime Details}
	\begin{table}
		...
		\begin{tabular}{lcc}
			...
			\midrule
			\textbf{ShapeDD-IDW} & \textbf{0.19} & \textbf{131,280} \\
			IDW-MMD & 0.20 & 130,500 \\
			KS Test & 0.36 & 28,161 \\
			D3 & 0.36 & 27,693 \\
