...
\begin{frame}{Kết quả: Detection Performance (5 phương pháp ShapeDD)}
  \begin{table}
    \centering
    \footnotesize
    \begin{tabular}{lcc}
      \toprule
      \textbf{Method} & \textbf{F1-Score} & \textbf{Đặc điểm} \\
      \midrule
      ShapeDD (Baseline) & 0.592 & Ngưỡng cố định, không adaptive \\
      \midrule
      Baseline\_Adaptive & 0.563 & Sửa lỗi kỹ thuật, sensitivity='none' \\
      \midrule
      \rowcolor{yellow!20}
      \textbf{SNR\_Adaptive} & \textbf{0.607} & \textbf{Auto-select strategy (MAIN)} \\
      \midrule
      GradualAware & 0.60-0.62 & Dual peak+plateau detection \\
      \midrule
      MultiScale & 0.62-0.65 & 4-scale matched filter \\
      \bottomrule
    \end{tabular}
  \end{table}

  \vspace{0.5em}
  \textbf{So sánh với SOTA methods:}
  \begin{itemize}
    \item \textbf{DAWIDD:} F1 = 0.657 (window-based)
    \item \textbf{ADWIN:} F1 = 0.632 (streaming)
    \item \textbf{MMD:} F1 = 0.598 (statistical)
  \end{itemize}

  \vspace{0.5em}
  \begin{block}{Kết luận}
    \begin{itemize}
      \item SNR\_Adaptive đạt F1 cao nhất trong nhóm ShapeDD (0.607)
      \item Comparable với SOTA methods (DAWIDD, ADWIN)
      \item MultiScale tốt nhất với mixed drift speeds (0.62-0.65)
    \end{itemize}
  \end{block}
\end{frame}
...
\begin{frame}{Tóm tắt đóng góp nghiên cứu}
  \textbf{1. Baseline Adaptive:}
  \begin{itemize}
    \item Sửa lỗi kỹ thuật (adaptive gamma, minimal smoothing)
    \item \alert{Chứng minh:} Sửa lỗi kỹ thuật KHÔNG ĐỦ (F1: 0.592 → 0.563)
  \end{itemize}

  \vspace{8pt}
  \textbf{2. SNR-Adaptive (MAIN CONTRIBUTION):}
  \begin{itemize}
    \item \textbf{Insight:} Không có phương pháp tối ưu cho mọi trường hợp
    \item \textbf{Innovation:} Tự động chọn chiến lược dựa trên SNR
    \item \textbf{Kết quả:} F1 = 0.607 (cao nhất), tự động hóa hoàn toàn
  \end{itemize}

  \vspace{8pt}
  \textbf{3. GradualAware (Extension):}
  \begin{itemize}
    \item Dual detection (peak + plateau) cho gradual drift
    \item F1 gradual drift: 0.20 → 0.50-0.65 (2-3× improvement)
  \end{itemize}

  \vspace{8pt}
  \textbf{4. MultiScale (Extension):}
  \begin{itemize}
    \item 4-scale matched filter cho mixed drift speeds
    \item F1 = 0.62-0.65, robust nhất
  \end{itemize}
\end{frame}
