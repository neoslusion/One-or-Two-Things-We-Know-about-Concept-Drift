...
Nghiên cứu tích hợp phương pháp \textbf{ADW-MMD} (Adaptive Density-Weighted MMD), lấy cảm hứng từ bộ ước lượng "Optimally-Weighted MMD" được đề xuất bởi Bharti et al.~\cite{bharti2023owmmd}. Trong khi Bharti gọi là "Optimally-Weighted" vì mục tiêu giảm phương sai ước lượng, luận văn sử dụng thuật ngữ "Adaptive Density-Weighted" để mô tả chính xác cơ chế hoạt động ($w \propto 1/\sqrt{\text{density}}$) và tránh nhầm lẫn với các phương pháp tối ưu hóa độ mạnh kiểm định (test power optimization).

Công thức MMD bình phương có trọng số được định nghĩa lại như sau:
\begin{equation}
	\text{MMD}^2_{\text{ADW}} = \sum_{i,j} w^{XX}_{ij} k(x_i, x_j) + \sum_{i,j} w^{YY}_{ij} k(y_i, y_j) - 2\sum_{i,j} w^{XY}_{ij} k(x_i, y_j)
\end{equation}
...
Để cân bằng giữa tốc độ và độ chính xác, báo cáo đề xuất quy trình phát hiện hai giai đoạn cho ShapeDD\_ADW\_MMD:
