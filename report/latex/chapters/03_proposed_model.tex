...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\STATE
		\STATE \textbf{--- Bước 0: Growth Process (CDT-MSW Algorithm 2) ---}
		\STATE $\text{drift\_length} \leftarrow \text{GrowthProcess}(\sigma_s)$ \COMMENT{WR-based estimation}
		\IF{drift\_length $> 1$}
		...
		\ENDIF
		\STATE \textbf{--- Bước 4: Incremental vs Gradual (PCD) ---}
...
Các ngưỡng trong Algorithm~\ref{alg:se-cdt} được xác định qua phân tích thực nghiệm trên synthetic datasets và tinh chỉnh qua nhiều vòng lặp:
\begin{itemize}
	...
	\item \textbf{Sudden:} $n_p \leq 3$, WR $< 0.15$, SNR $> 2.0$. Sudden drift tạo peak sắc nét (WR thấp) với tỉ lệ tín hiệu/nhiễu cao (SNR $> 2$). Ngưỡng WR $= 0.15$ cho phép FWHM $< 30\%$ window size.
...
\end{itemize}
\textit{Ghi chú: Các ngưỡng được tối ưu trên synthetic data và có thể cần điều chỉnh cho từng domain cụ thể. Xem bảng kèm theo mã nguồn để biết giá trị chính xác.}
