...
Như đã phân tích trong Chương~2, ShapeDD gốc có hiệu suất phát hiện tốt nhưng gặp hạn chế về chi phí tính toán do permutation test. Cụ thể:
\begin{itemize}
    \item \textbf{Chi phí tính toán cao:} $O(N_{perm} \times n^2)$ với $N_{perm} = 2500$
    \item \textbf{Throughput thấp:} Chỉ đạt $\sim$6,800 samples/giây
    \item \textbf{Precision thấp:} 0.322 do nhiều false positives
\end{itemize}

Để giải quyết các vấn đề này, luận văn đề xuất hai cải tiến chính:
\begin{enumerate}
    \item \textbf{OW-MMD (Section~\ref{sec:ow-mmd}):} Sử dụng variance-reduction weighting để giảm số lượng bootstrap samples từ 2500 xuống 10, tăng throughput lên 47,000 samples/giây (gấp 7 lần)
    \item \textbf{MMD-Agg (Section~\ref{sec:mmdagg}):} Sử dụng aggregated kernel testing để cải thiện F1-score từ 0.440 lên 0.509 trên các dataset có drift mạnh
\end{enumerate}
...
\begin{table}[H]
\centering
\caption{So sánh hiệu suất ShapeDD và ShapeDD\_OW\_MMD (Kết quả benchmark)}
\label{tab:owmmd-performance}
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{ShapeDD (baseline)} & \textbf{ShapeDD\_OW\_MMD} & \textbf{Improvement} \\
\midrule
F1-score & 0.440 & \textbf{0.562} & +27.7\% \\
Precision & 0.322 & \textbf{0.573} & +78.0\% \\
Recall & 0.744 & 0.584 & -21.5\% \\
Throughput (samples/s) & 6,824 & \textbf{46,773} & 6.9× faster \\
Rank (out of 8) & 7 & \textbf{2} & +5 ranks \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
BUFFER\_SIZE & 750 & Số mẫu trong detection window \\
CHUNK\_SIZE & 150 & Tần suất kiểm tra drift (mỗi 150 mẫu) \\
SHAPE\_L1 & 50 & Cửa sổ nhỏ cho ShapeDD \\
SHAPE\_L2 & 150 & Cửa sổ lớn cho MMD comparison \\
SHAPE\_N\_PERM & 2500 & Số lần hoán vị cho permutation test \\
DRIFT\_PVALUE & 0.05 & Ngưỡng p-value phát hiện drift \\
ADAPTATION\_DELAY & 50 & Số mẫu chờ sau detection trước khi adapt \\
ADAPTATION\_WINDOW & 800 & Số mẫu sử dụng để retrain model \\
\bottomrule
\end{tabular}
\end{table}
