...
\textit{Tóm tắt: Trên cơ sở lý thuyết về ShapeDD và các phương pháp phát hiện concept drift đã trình bày ở Chương~\ref{chap:theoretical-foundation}, chương này trình bày việc tích hợp các phương pháp MMD cải tiến gần đây (OW-MMD, MMD-Agg) vào framework ShapeDD nhằm cải thiện hiệu suất tính toán, đồng thời đề xuất phương pháp mới SHAPED\_CDT cho phân loại drift không giám sát. Báo cáo cũng trình bày kiến trúc hệ thống xử lý luồng dữ liệu thời gian thực dựa trên Apache Kafka. Các đóng góp chính bao gồm: (1) Tích hợp Optimally-Weighted MMD (OW-MMD) để tăng tốc độ xử lý, (2) Xây dựng khung chiến lược thích ứng đa dạng dựa trên loại drift, và (3) Thiết kế kiến trúc hệ thống có khả năng mở rộng và chịu lỗi cao.}
...
\subsubsection{SE-CDT: Phân loại drift không giám sát}

Phương pháp CDT\_MSW gốc~\cite{guo2022cdtmsw} yêu cầu \textit{labels} để tính toán accuracy ratio, điều này không phù hợp trong môi trường streaming không giám sát. Luận văn đề xuất \textbf{SE-CDT (ShapeDD-Enhanced CDT)}, một phương pháp thay thế sử dụng trực tiếp tín hiệu drift magnitude $\sigma(t)$ từ ShapeDD để phân loại drift type.

\textbf{Cơ sở lý thuyết:} Theo phân loại của CDT\_MSW, drift được chia thành:
\begin{itemize}
...
\textbf{Đặc trưng phân loại từ tín hiệu $\sigma(t)$:}
\begin{enumerate}
	\item \textbf{Peak Width Ratio:} $\text{WR} = \frac{\text{FWHM}}{2L}$ — Tỷ lệ chiều rộng peak so với window. TCD có WR $< 1.5$, PCD có WR $> 2.0$.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\STATE Làm mịn tín hiệu: $\sigma_s \leftarrow \text{GaussianFilter}(\sigma, \sigma=2)$
		...
		\IF{$n_p \leq 4$ AND $\text{WR} < 1.2$ AND $\text{SNR} > 4$}
		\RETURN \textbf{SUDDEN} (TCD)
		\ELSIF{$n_p \geq 4$ AND $\text{CV} < 0.2$}
		\RETURN \textbf{RECURRENT} (PCD)
		\ELSIF{$\text{WR} > 2.0$}
		\RETURN \textbf{GRADUAL} (PCD)
		\ELSIF{$n_p \geq 10$ AND $\text{WR} > 5$}
...
\textbf{Ưu điểm của SE-CDT (SHAPED\_CDT):}
\begin{itemize}
	...
	\item \textbf{Sử dụng standard MMD:} Phù hợp hơn OW-MMD cho classification vì standard MMD giữ lại tất cả sự khác biệt bao gồm cả những thay đổi nhỏ từ PCD (Gradual, Incremental).
\end{itemize}

\textbf{Lưu ý về biến thể OW-MMD:}
Luận văn cũng thử nghiệm sử dụng OW-MMD thay cho standard MMD trong SHAPED\_CDT. Tuy nhiên, kết quả cho thấy OW-MMD không phù hợp cho classification do cơ chế variance reduction loại bỏ những thay đổi nhỏ (coi là noise). OW-MMD phù hợp hơn cho drift detection, nhưng standard MMD tốt hơn cho classification. Chi tiết thử nghiệm và so sánh định lượng được trình bày ở Chương~\ref{chap:experiments}.
