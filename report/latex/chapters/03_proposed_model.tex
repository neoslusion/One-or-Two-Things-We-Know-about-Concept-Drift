...
\begin{equation}
\text{MMD}^2_{\text{OW}} = \sum_{i,j} w^{XX}_{ij} k(x_i, x_j) + \sum_{i,j} w^{YY}_{ij} k(y_i, y_j) - \text{MMD}^2_{\text{OW}} = \sum_{i,j} w^{XX}_{ij} k(x_i, x_j) + \sum_{i,j} w^{YY}_{ij} k(y_i, y_j) - 2\sum_{i,j} w^{XY}_{ij} k(x_i, y_j)
\end{equation}
...
Để khắc phục, chúng tôi tích hợp kỹ thuật \textbf{MMD-Agg} (MMD Aggregated)~	hione{schrab2023mmdagg}. Thay vì chọn một $\sigma$ cố định (thường dùng median heuristic), MMD-Agg tính toán MMD trên một tập hợp các bandwidths khác nhau $\{\sigma_1, \sigma_2, \dots, \sigma_k\}$ và tổng hợp kết quả.
