...
\begin{itemize}
	...
	\item \textbf{Tầng chiến lược:} Xây dựng khung thích ứng thông minh, sử dụng kết quả phân loại từ \textit{SE-CDT} để kích hoạt chiến lược cập nhật mô hình tối ưu (Reset, Incremental, hoặc Reuse).
...
Trong ShapeDD gốc, thống kê MMD được tính toán dựa trên trọng số đều (uniform weights), coi vai trò của mọi điểm dữ liệu trong cửa sổ là như nhau. Nhắc lại từ Chương~\ref{chap:theoretical-foundation}, công thức MMD chuẩn có dạng:
\begin{equation}
...
\end{equation}
trong đó $w_i$ và $v_p$ là trọng số của từng điểm (thỏa mãn $\sum_i w_i = 1$, $\sum_p v_p = 1$).

\textbf{Câu hỏi:} Làm sao tính $w_i$ để phát hiện drift hiệu quả?
...
\end{itemize}
Lý do: Drift thường xuất hiện đầu tiên ở biên phân phối, nên ta cần "lắng nghe" các điểm ở đó kỹ hơn.

\textbf{Tại sao dùng căn bậc hai ($1/\sqrt{d_i}$) thay vì nghịch đảo thẳng ($1/d_i$)?}
\begin{itemize}
	\item Nếu dùng $1/d_i$: Trọng số quá lớn cho outliers, dễ bị ảnh hưởng bởi nhiễu.
...
Khi dùng MMD chuẩn (uniform weights), tất cả các cặp điểm đóng góp ngang nhau vào kết quả. Vấn đề: Các điểm ở vùng đông đúc thường "giống nhau" (highly correlated), dẫn đến kết quả dao động nhiều.

IDW-MMD khắc phục bằng hai cách:
\begin{enumerate}
...
\textbf{Kết quả thực tế:} Độ lệch chuẩn (standard deviation) của $\widehat{\text{MMD}}^2_{\text{IDW}}$ giảm khoảng 30\% so với uniform MMD trên dữ liệu không có drift. Nhờ vậy, ta có thể dùng asymptotic p-value (nhanh) thay vì permutation test (chậm) mà vẫn đảm bảo độ chính xác.
...
\textbf{1. Không hiệu quả với Mild Drift (drift nhẹ):}
\begin{itemize}
	\item IDW-MMD có xu hướng "lọc bỏ" những thay đổi nhỏ ở vùng trung tâm, coi chúng là nhiễu.
	\item \textbf{Kết quả:} F1-score chỉ đạt 0.006 trên dataset \texttt{gen\_random\_mild}, trong khi MMD chuẩn đạt 0.585 (xem chi tiết tại Mục~\ref{sec:detection-eval}).
	\item \textbf{Nguyên nhân:} Mild drift thường xảy ra ở vùng trung tâm (nơi có nhiều điểm), nhưng IDW-MMD giảm trọng số của các điểm này.
\end{itemize}

\textbf{2. Không phù hợp cho phân loại loại drift:}
\begin{itemize}
	\item IDW-MMD thiết kế cho bài toán \textit{phát hiện} (có drift hay không), không phải \textit{phân loại} (loại drift gì).
	\item Cơ chế giảm nhiễu có thể loại bỏ luôn tín hiệu của Gradual/Incremental drift (diễn ra chậm).
	\item \textbf{Giải pháp:} Module SE-CDT (Mục~\ref{sec:shaped-cdt}) dùng MMD chuẩn thay vì IDW-MMD để giữ lại đầy đủ thông tin.
\end{itemize}

\textbf{3. Khi nào nên dùng IDW-MMD:}
\begin{itemize}
	\item \textbf{Phù hợp:} Sudden drift mạnh ($\delta \geq 1.0$), đặc biệt khi cần xử lý nhanh (high throughput).
	\item \textbf{Cẩn thận:} Mild drift ($\delta < 0.5$), gradual drift diễn ra ở vùng trung tâm.
	\item \textbf{Lời khuyên:} Kết hợp với MMD chuẩn hoặc adaptive threshold để bù đắp hạn chế.
\end{itemize}
