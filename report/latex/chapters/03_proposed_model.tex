...
Luận văn đề xuất phương pháp \textbf{IDW-MMD} (Inverse Density-Weighted MMD), một \textbf{heuristic mới do luận văn xây dựng}, lấy động lực từ ý tưởng weighted MMD trong ``Optimally-Weighted MMD'' của Bharti et al.~\cite{bharti2023owmmd}.

\textbf{Lưu ý quan trọng:} Phương pháp của Bharti et al. (2023) được thiết kế cho bài toán suy luận thống kê (likelihood-free inference) và đề xuất trọng số tối ưu phức tạp, \textbf{không liên quan đến bài toán drift detection}. Luận văn này chỉ kế thừa ý tưởng ``gán trọng số cho từng điểm dữ liệu trong MMD'' và đề xuất một \textbf{heuristic đơn giản riêng} --- trọng số nghịch biến với căn bậc hai của mật độ kernel --- để phù hợp với bài toán drift detection trong streaming.

Cụ thể, trọng số $w_i$ được tính dựa trên mật độ cục bộ (local density):
\begin{equation}
	w_i \propto \frac{1}{\sqrt{\sum_{j} k(x_i, x_j)} + \epsilon} \quad \text{(Inverse Density Weighting)}
\end{equation}
