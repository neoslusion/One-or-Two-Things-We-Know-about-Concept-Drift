...
\begin{itemize}
	\item \textbf{Tích hợp ShapeDD\_WMMD\_PROPER:} Tích hợp thành công phương pháp IDW-MMD (Inverse Density-Weighted MMD) với asymptotic p-value, lấy cảm hứng từ~\cite{bharti2023owmmd}, vào ShapeDD detector. Kết quả cho thấy phương pháp này cải thiện thông lượng xử lý gấp \textbf{17--20 lần} (đạt $\approx$ 111,000--131,000 mẫu/giây) so với ShapeDD gốc ($\approx$ 8,000 mẫu/giây), trong khi đạt F1-score = 0.481 với \textbf{0 False Positives}.
	\item \textbf{Đề xuất SE-CDT:} Xây dựng phương pháp phân loại drift không giám sát \textbf{SE-CDT}, tận dụng khả năng định vị chính xác thời điểm drift của ShapeDD để phân loại loại drift. Phương pháp đạt độ chính xác \textbf{85.8\%} trong việc phân loại nhóm drift (TCD vs PCD), vượt trội so với CDT\_MSW supervised (38.7\%), tạo tiền đề cho chiến lược thích ứng thông minh.
\end{itemize}

\subsection{Đóng góp về Thực nghiệm (Empirical)}
\begin{itemize}
	\item \textbf{Benchmark toàn diện:} Thực hiện đánh giá có hệ thống 8 phương pháp phát hiện drift (MMD, MMD\_ADW, KS, ShapeDD, ShapeDD\_WMMD\_PROPER, SE\_CDT, D3, DAWIDD) trên 11 tập dữ liệu tổng hợp đa dạng. Kết quả được kiểm chứng thống kê bằng kiểm định Friedman và Nemenyi (với 30 lần chạy độc lập, $\alpha = 0.05$), cung cấp cái nhìn khách quan về hiệu năng của các thuật toán hiện đại.
	...
	\item \textbf{Xác nhận hành vi đúng:} Benchmark xác nhận SE\_CDT/WMMD\_PROPER không phát hiện ``drift'' trên các dataset virtual drift (hyperplane, led\_abrupt, sea) --- hành vi mong đợi vì $P(X)$ không đổi.
\end{itemize}
...
\begin{itemize}
	...
	\item Nếu ưu tiên \textbf{FP thấp}: Sử dụng SE-CDT (ADW) hoặc ShapeDD\_WMMD\_PROPER (FP = 0 trong detection benchmark)
\end{itemize}

\subsection{Subcategory Accuracy còn hạn chế}
Mặc dù Category Accuracy (TCD vs PCD) đạt 85.8\%, Subcategory Accuracy chỉ đạt 46.6\%. Cụ thể:
\begin{itemize}
...
Hành trình giải quyết bài toán concept drift là một quá trình liên tục. Luận văn này đã đóng góp một bước tiến cụ thể thông qua:
\begin{enumerate}
	\item \textbf{Cải thiện hiệu năng:} Tích hợp IDW-MMD với asymptotic p-value giúp tăng throughput \textbf{17--20 lần} (từ 8,000 lên 131,000 mẫu/giây), biến ShapeDD thành giải pháp production-ready.
	\item \textbf{Đảm bảo độ tin cậy:} ShapeDD\_WMMD\_PROPER và SE\_CDT đạt \textbf{0 False Positives} trong detection benchmark, quan trọng cho môi trường production.
	\item \textbf{Phân loại không giám sát:} SE-CDT đạt \textbf{CAT = 85.8\%} mà không cần labels, vượt trội CDT\_MSW supervised (38.7\%).
\end{enumerate}
