...
\begin{enumerate}
	...
	\item \textbf{Đảm bảo độ tin cậy:} ShapeDD\_WMMD\_PROPER và SE\_CDT đạt \textbf{0 False Positives} trong detection benchmark (trên các dataset có thay đổi $P(X)$), quan trọng cho môi trường production.
	\item \textbf{Phân loại không giám sát:} SE-CDT đạt \textbf{CAT = 85.8\%} mà không cần labels, cao hơn đáng kể so với CDT\_MSW supervised (38.7\%) trong điều kiện thử nghiệm cụ thể (window\_size = 50).
\end{enumerate}
