...
\begin{itemize}
	...
	\item \textbf{Phân tích độ trễ và chi phí:} Chứng minh hiệu quả của việc giảm số lượng hoán vị (permutation) nhờ cơ chế giảm phương sai của ADW-MMD, giúp giảm độ trễ phát hiện trung bình xuống còn 18 mẫu.
\end{itemize}
...
\begin{itemize}
	...
	\item \textbf{Khung chiến lược thích ứng:} Hiện thực hóa 5 chiến lược thích ứng tương ứng với các loại drift (Sudden, Incremental, Gradual, Recurrent, Blip). Đặc biệt, chiến lược \textit{tri???n khai m?? h??nh ????ng b??ng (Frozen Model Deployment)} được đề xuất để đo lường chính xác tác động của drift trong môi trường production.
\end{itemize}
...
\begin{enumerate}
	...
	\item \textbf{Trade-off giữa Tốc độ và Độ chính xác:} Các phương pháp cửa sổ trượt truyền thống chính xác nhưng chậm. Việc tích hợp ADW-MMD đã giải quyết nút thắt này, đưa tốc độ xử lý của phương pháp dựa trên kernel tiệm cận với các thuật toán thống kê nhẹ (như ADWIN) mà không hy sinh độ chính xác.
...
\subsection{False Positives cao}
Phương pháp SHAPED\_CDT (Standard MMD) đạt recall 98\% nhưng có số lượng False Positives cao (FP = 1513 trong tổng benchmark). Điều này dẫn đến Precision chỉ đạt 25\%. Trong môi trường production, số lượng báo động giả cao có thể gây quá tải cho hệ thống thích ứng và làm giảm độ tin cậy. Việc phát triển cơ chế post-filtering hoặc adaptive threshold là hướng cải thiện cần thiết.

\subsection{Subcategory Accuracy còn hạn chế}
Mặc dù Category Accuracy (TCD vs PCD) đạt 74.6\%, Subcategory Accuracy chỉ đạt 42\%. Cụ thể:
\begin{itemize}
...
Hành trình giải quyết bài toán concept drift là một quá trình liên tục. Luận văn này đã đóng góp một bước tiến cụ thể thông qua việc (1) tích hợp các tiến bộ lý thuyết mới nhất (ADW-MMD) để giải quyết bài toán hiệu năng, và (2) đề xuất hướng tiếp cận phân loại drift không giám sát (SHAPED\_CDT) để định hướng thích ứng.
