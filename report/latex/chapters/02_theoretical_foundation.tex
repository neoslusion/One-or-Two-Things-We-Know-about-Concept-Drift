...
Sau khi khảo sát các phương pháp phát hiện drift ở Chương~2, chương này trình bày nền tảng lý thuyết 
của phương pháp ShapeDD --- phương pháp được lựa chọn làm cơ sở phát triển cho luận văn. 
Chương bắt đầu với định nghĩa formal về concept drift, sau đó đi sâu vào lý thuyết MMD và cơ chế phát hiện của ShapeDD.
...
Cụ thể hơn, sự trôi dạt khái niệm là một vấn đề trong đó các mối quan hệ thống kê giữa các giá trị đầu vào và giá trị mục tiêu bị thay 
đổi theo thời gian theo cách không thể dự đoán được~\cite{schlimmer1986incremental}.

Sự trôi dạt khái niệm có thể dẫn đến hiệu suất giảm trong quá trình vận hành thực tế của mô hình học máy, do bản 
...
Có nhiều loại trôi dạt khác nhau, tùy thuộc vào các yếu tố dữ liệu đang thay đổi. Các loại chính của sự trôi dạt khái 
niệm bao gồm~\cite{sciencedirect2024drift, hovakimyan2024evolving}.
...
Sự trôi dạt khái niệm có thể ảnh hưởng lớn đến hiệu suất của mô hình dự đoán, đặc biệt là khi mô hình học từ luồng dữ liệu. 
Một loạt các dịch vụ/ứng dụng trong bối cảnh hệ thống và mạng truyền thông có thể bị cản trở bởi sự trôi dạt khái niệm như~\cite{ramakrishnan2014enabling}:
...
\section{Phương pháp ShapeDD: Phương pháp phát hiện trôi dạt dựa trên hình dạng và khử nhiễu tín hiệu}
...
Kết quả lý thuyết trung tâm của ShapeDD được phát biểu trong Định lý~1, mô tả \textit{hình dạng đặc trưng, không phụ thuộc vào phân phối} 
của tín hiệu $\sigma$ khi xảy ra một trôi dạt đột ngột.
...
Hàm $h_l(t)$ có dạng \textbf{tam giác cân}, đạt cực đại tại thời điểm xảy ra drift, và suy giảm tuyến tính về hai phía. 
Đặc tính này hoàn toàn không phụ thuộc vào dạng phân phối hay độ đo khoảng cách sử dụng, mà chỉ bị co giãn theo hệ số $\|P - Q\|$.
...
Tóm lại, ShapeDD tiếp cận bài toán phát hiện trôi dạt khái niệm từ góc nhìn của \textbf{xử lý tín hiệu}. 
...
Như đã nhắc đến ở trên, ShapeDD dựa trên Maximum Mean Discrepancy (MMD)~\cite{gretton2012kernel}, 
một thước đo thống kê được sử dụng để so sánh hai phân phối xác suất $P$ và $Q$. Ý tưởng cốt lõi là ánh xạ dữ liệu từ không gian 
gốc vào không gian feature cao chiều nơi việc so sánh trở nên nhạy cảm hơn với sự khác biệt phân phối.

\subsubsection{Định nghĩa:} MMD đo lường khoảng cách giữa hai phân phối bằng cách tìm hàm $f$ có thể phân biệt tốt nhất giữa chúng. 
...
\textbf{Intuition:} Trong không gian ban đầu, hai phân phối có thể khó phân biệt. Bằng cách ánh xạ vào RKHS thông qua hàm kernel~\cite{scholkopf2002learning}, 
...
\textbf{Ví dụ minh họa với tính toán cụ thể:} Xét hai tập mẫu nhỏ từ hai phân phối 1D:
\begin{itemize}
    \item $X = \{0.1, -0.3, 0.5\}$ từ $P = \mathcal{N}(0, 1)$
    \item $Y = \{1.8, 2.3, 2.0\}$ từ $Q = \mathcal{N}(2, 1)$
\end{itemize}

Sử dụng Gaussian kernel với $\sigma = 1$: $k(x, y) = \exp(-\frac{(x-y)^2}{2})$

\textit{Bước 1:} Tính các thành phần kernel:
\begin{align*}
\frac{1}{n^2}\sum_{i,j} k(x_i, x_j) &= \frac{1}{9}[k(0.1,0.1) + k(0.1,-0.3) + \ldots] \approx 0.92 \\
\frac{1}{m^2}\sum_{i,j} k(y_i, y_j) &= \frac{1}{9}[k(1.8,1.8) + k(1.8,2.3) + \ldots] \approx 0.94 \\
\frac{2}{nm}\sum_{i,j} k(x_i, y_j) &= \frac{2}{9}[k(0.1,1.8) + k(0.1,2.3) + \ldots] \approx 0.21
\end{align*}

\textit{Bước 2:} Tính $\widehat{\text{MMD}}^2$:
\[
\widehat{\text{MMD}}^2 = 0.92 + 0.94 - 0.21 = 1.65 > 0
\]

Kết quả dương ($\widehat{\text{MMD}}^2 = 1.65$) cho thấy hai mẫu có phân phối khác nhau. Giá trị này có thể dùng để kiểm định với permutation test: nếu hai phân phối giống nhau, $\widehat{\text{MMD}}^2$ sẽ gần 0.
