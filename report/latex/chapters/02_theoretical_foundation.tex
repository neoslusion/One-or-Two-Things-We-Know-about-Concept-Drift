...
ShapeDD tiếp cận bài toán phát hiện concept drift từ góc nhìn xử lý tín hiệu: độ lớn thay đổi của phân phối khi drift xảy ra luôn tạo ra một tín hiệu có hình tam giác đặc trưng.
...
\textbf{Lựa chọn kích thước cửa sổ $l_1$:} Đây là tham số quan trọng nhất của ShapeDD; cửa sổ nhỏ nhạy với drift nhanh nhưng dễ bị nhiễu, cửa sổ lớn thì ổn định hơn nhưng phát hiện trễ. Khi cần điều chỉnh tự động, có thể đặt $l_1$ tỉ lệ với độ dài luồng. Giá trị cụ thể dùng trong luận văn được liệt kê ở bảng cấu hình thực nghiệm (Chương~\ref{chap:experiments}).
...
\textbf{Lựa chọn bandwidth $\gamma$:} Có thể sử dụng median heuristic để tự động chọn $\gamma$:
\begin{equation}
...
\textbf{Hạn chế của permutation test:} Số lần resampling $N_{\text{perm}}$ thường lấy vài nghìn để p-value có độ phân giải tốt, và mỗi lần phải tính lại kernel matrix, khiến chi phí kiểm định lớn hơn nhiều lần so với chi phí tính MMD thô. Độ chính xác thống kê của ngưỡng phát hiện cũng chỉ đảm bảo nếu phân phối dưới giả thuyết không trôi dạt được ước lượng phù hợp.

\textbf{Động lực cải tiến:} Hạn chế này là động lực cho việc đề xuất \textbf{Weighted MMD} kết hợp với p-value dựa trên xấp xỉ Gamma thay cho permutation test, được trình bày trong Chương~\ref{chap:proposed-model}. Phương pháp này lấy cảm hứng từ ý tưởng ``Optimally-Weighted MMD'' của Bharti et al.~\cite{bharti2023owmmd}, dùng một heuristic đơn giản hơn (trọng số nghịch biến với mật độ kernel), với kỳ vọng giữ được độ chính xác phát hiện và đem lại kiểm định có cơ sở thống kê rõ ràng hơn.
