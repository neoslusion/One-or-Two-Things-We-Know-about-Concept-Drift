...
\textit{Tóm tắt: Sau khi khảo sát các phương pháp phát hiện drift ở Chương~2, chương này trình bày nền tảng lý thuyết
...
liệu thường thay đổi theo thời gian — hiện tượng này được gọi là \textbf{trôi dạt khái niệm (concept drift)}. Khi đó, các mô hình học
...
Phương pháp \textbf{ShapeDD (Shape-Based Drift Detection)} được đề xuất nhằm giải quyết vấn đề này thông qua việc khai thác \textit{đặc trưng hình học} của tín hiệu trôi dạt, đặc biệt hiệu quả trong các trường hợp \textbf{drift đột ngột (abrupt drift)}.
...
\textit{Quy ước ký hiệu}: Luận văn dùng $\sigma(t)$ (hoặc $\sigma_{d,l}$) cho drift magnitude signal, và $\gamma$ cho kernel bandwidth của RBF kernel --- ký hiệu nhất quán với cách đặt tên trong code \texttt{gamma}. Trong các phần lý thuyết trích từ bài báo gốc, $\sigma_k$ hoặc $\sigma$ với subscript vẫn được giữ nguyên và có thể được hiểu là $\gamma$.
...
\textbf{Hạn chế về throughput và động lực cải tiến:} Với $N_{\text{perm}} = 2500$ (giá trị mặc định) và $l_1 = 50$, mỗi window cần thực hiện khoảng $2500 \times 50^2 = 6.25$ triệu phép tính kernel. Điều này giới hạn throughput của ShapeDD gốc ở mức khoảng $6{,}800$ mẫu/giây --- chưa đáp ứng được yêu cầu của các ứng dụng thời gian thực xử lý hàng chục nghìn mẫu mỗi giây. Hạn chế này là động lực chính cho việc đề xuất phương pháp cải tiến \textbf{Weighted MMD} với công thức phân phối tiệm cận (asymptotic distribution) thay vì permutation test, được trình bày trong Chương~\ref{chap:proposed-model}. Phương pháp này lấy cảm hứng từ ý tưởng ``Optimally-Weighted MMD'' của Bharti et al.~\cite{bharti2023owmmd}, nhưng sử dụng một heuristic đơn giản hơn --- trọng số nghịch biến với mật độ kernel --- để cải thiện tốc độ xử lý trong khi vẫn duy trì độ chính xác phát hiện.
...
\begin{enumerate}
	\item \textbf{Khử nhiễu và giảm báo động giả:} Nhờ cơ chế lọc shape, ShapeDD giảm đáng kể số lần báo động sai do nhiễu thống kê. Thay vì phản ứng với mọi dao động nhỏ trong tín hiệu, phương pháp chỉ tập trung vào các biến động có hình dạng phù hợp với drift thật. Việc yêu cầu tín hiệu phải khớp với mẫu hình tam giác giúp lọc bỏ các dao động ngẫu nhiên, cải thiện tính ổn định trong môi trường nhiều nhiễu so với phương pháp dùng trực tiếp MMD không lọc shape.
	...
	\item \textbf{Tính toán hiệu quả:} Thuật toán ShapeDD được triển khai tối ưu để chạy online. Bước convolution/shape detection có chi phí $O(l)$, trong khi bước tính kernel matrix có chi phí $O(l^2)$ cho mỗi cửa sổ. Tổng thể, ShapeDD đủ nhẹ để áp dụng trong thời gian thực trên luồng dữ liệu.
\end{enumerate}
