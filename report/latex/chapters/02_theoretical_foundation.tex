...
\textbf{Hình dạng tam giác đặc trưng:} Khi xảy ra drift đột ngột tại thời điểm $t_0$, độ lớn drift $\sigma(t)$ có hình dạng đặc trưng dạng tam giác: tăng lên rồi giảm xuống trong khoảng thời gian tương ứng với độ dài cửa sổ $l$. Tính chất này được chứng minh chính thức trong Theorem sau:

\begin{theorem}[Triangle Shape Property - Hinder et al.~\cite{hinder2024survey_partA}]
\label{thm:triangle-shape}
Cho $\|\cdot\|$ là một norm trên không gian các phân phối hữu hạn, $l > 0$, và $p_t$ là drift process với single abrupt drift tại $t = 0$, tức là:
\[
p_t = \begin{cases}
P & t \leq 0 \\
Q & t > 0
\end{cases}
\]
trong đó $P, Q$ là các phân phối tùy ý. Khi đó:
\[
\sigma_{\|\cdot\|,l,p_{\cdot}}(t) = \|P - Q\| \cdot \max\left(0, 1 - \frac{|l - t|}{l}\right) =: \|P - Q\| \cdot h_l(t)
\]

Đặc biệt, $\sigma$ có thể phân tích thành:
\begin{itemize}
    \item \textbf{Scalar part:} $\|P - Q\|$ - phụ thuộc vào norm và distributions nhưng độc lập với $l$
    \item \textbf{Shape part:} $h_l(t) = \max(0, 1 - |l-t|/l)$ - chỉ phụ thuộc vào $l$, độc lập với norm và distributions
\end{itemize}
\end{theorem}

\textbf{Hệ quả cho multiple drifts:} Theorem 1 mở rộng cho drift processes với nhiều abrupt drifts tại $t_1 < ... < t_n$:
\[
p_t = \begin{cases}
P_0 & t < t_1 \\
P_1 & t \in [t_1, t_2) \\
...
Với mọi $l < \min\{t_{i+1} - t_i\}/2$ (drifts đủ xa nhau), ta có:
\[
\sigma_{\|\cdot\|,l,p_{\cdot}}(t) = \sum_{i=1}^{n} \|P_{i-1} - P_i\| \cdot h_l(t - t_i)
\]

\textbf{Ý nghĩa thực tiễn:} Theorem 1 cho thấy shape $h_l(t)$ là universal pattern cho abrupt drift, không phụ thuộc vào đặc trưng cụ thể của distributions. Điều này cho phép:
\begin{itemize}
    \item Sử dụng chung tham số shape cho nhiều norms và window lengths khác nhau
    \item Denoising robust bằng cách fit parametric curve $\sum_i s_i h_{l_i}(t - t_i)$ thay vì pointwise estimation
    \item Precise localization của drift time $t_0$ thông qua zero-crossing detection của $h'_l$
\end{itemize}

\textbf{Phạm vi của luận văn:} Theorem 1 chứng minh rigorous triangle shape property cho \textbf{single abrupt drift}. Luận văn này tập trung nghiên cứu chuyên sâu sudden drift scenarios, nơi mà theoretical guarantees mạnh nhất. Các loại drift khác (gradual, incremental) không có closed-form characterization tương tự, đòi hỏi approaches khác và nằm ngoài scope của nghiên cứu này. Việc focus vào sudden drift cho phép thorough analysis với strong mathematical foundation thay vì shallow coverage nhiều types.
