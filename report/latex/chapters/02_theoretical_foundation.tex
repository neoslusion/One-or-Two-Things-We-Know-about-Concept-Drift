...
\textbf{Hạn chế của permutation test và động lực cải tiến:} Số lần resampling $N_{\text{perm}}$ thường lấy ở mức vài nghìn để p-value có độ phân giải tốt; mỗi lần lại phải tính lại kernel matrix, làm cho chi phí của bước kiểm định lớn hơn nhiều lần so với chi phí tính MMD thô. Quan trọng hơn, độ chính xác thống kê của ngưỡng phát hiện chỉ đảm bảo nếu null distribution được mô phỏng đúng. Hạn chế này là động lực cho việc đề xuất \textbf{Weighted MMD} kết hợp với p-value tiệm cận (Gamma-approximation null) thay cho permutation test, được trình bày trong Chương~\ref{chap:proposed-model}. Phương pháp này lấy cảm hứng từ ý tưởng ``Optimally-Weighted MMD'' của Bharti et al.~\cite{bharti2023owmmd}, dùng một heuristic đơn giản hơn (trọng số nghịch biến với mật độ kernel), với kỳ vọng giữ được độ chính xác phát hiện và đem lại p-value được hiệu chỉnh đúng theo lý thuyết.
