...
Để đánh giá hiệu quả của ShapeDD trên các kịch bản drift khác nhau, được tạo ra các bộ dữ liệu tổng hợp được kiểm soát với đặc trưng drift được định nghĩa chính xác. Cách tiếp cận này cho phép nghiên cứu đánh giá hiệu suất thuật toán dưới các điều kiện đã biết và phân tích tác động của các tham số khác nhau.

\textbf{Bộ dữ liệu Abrupt Drift:} Nghiên cứu tạo ra các bộ dữ liệu với thay đổi đột ngột, tức thì trong phân phối dữ liệu. Bộ dữ liệu bao gồm 10,000 điểm dữ liệu với sampling đồng nhất trong không gian đơn vị. Drift được giới thiệu bằng cách dịch chuyển các tham số phân phối tại các điểm thời gian được chọn ngẫu nhiên:
...
\textbf{Bộ dữ liệu Incremental Drift:} Nghiên cứu tạo ra các bộ dữ liệu thể hiện thay đổi dần dần, liên tục trong các tham số phân phối theo thời gian. Điều này đại diện cho các mẫu drift tiến hóa chậm thường được tìm thấy trong các ứng dụng thực tế:
...
\textbf{Biến thiên tham số:} Đối với mỗi loại drift, được tạo ra nhiều biến thể dataset với:
\begin{itemize}
...
\textbf{Đánh giá Prequential:} Nghiên cứu sử dụng cách tiếp cận test-then-train để đánh giá streaming thực tế:
...
\textbf{Metric hiệu suất:} Nghiên cứu sử dụng nhiều metric để đánh giá các khía cạnh khác nhau của hiệu suất:
...
\textbf{Kiểm định ý nghĩa:} Nghiên cứu sử dụng các test thống kê phù hợp để xác minh ý nghĩa của sự khác biệt hiệu suất:
...
\textbf{Phân tích Effect Size:} Ngoài ý nghĩa thống kê, được tính effect size để đánh giá ý nghĩa thực tế:
...
Luận văn phát triển framework phần mềm toàn diện cho thí nghiệm concept drift:
...
Cross-validation truyền thống không phù hợp cho dữ liệu thời gian có drift. Nghiên cứu sử dụng temporal cross-validation:
...
Tất cả dataset thực tế được sử dụng trong nghiên cứu này hoặc được công khai hoặc được ẩn danh hóa đúng cách. Nghiên cứu đảm bảo tuân thủ các quy định bảo vệ dữ liệu có liên quan.

\subsection{Khả năng tái tạo}

Luận văn cam kết công khai code, dataset và cấu hình thí nghiệm để cho phép tái tạo và tạo thuận lợi cho nghiên cứu tương lai.

\section{Tóm tắt}

Chương này đã phác thảo phương pháp luận toàn diện được sử dụng trong nghiên cứu này. Nghiên cứu này kết hợp phát triển lý thuyết với thiết kế thuật toán thực tế và đánh giá thực nghiệm nghiêm ngặt. Chương tiếp theo trình bày kết quả của việc áp dụng phương pháp luận này để điều tra phát hiện concept drift và thích ứng trên nhiều lĩnh vực và kịch bản.
