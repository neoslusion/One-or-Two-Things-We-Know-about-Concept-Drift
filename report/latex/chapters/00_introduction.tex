...
Bài toán đặt ra có thể phát biểu chính xác hơn như sau: concept drift có thể xảy ra ở hai cấp độ --- (1) phân phối đầu vào $P(X)$ thay đổi (covariate shift), hoặc (2) mối quan hệ $P(y|X)$ thay đổi trong khi $P(X)$ giữ nguyên (real concept drift). Luận văn này tập trung vào hướng không giám sát, tức là phát hiện sự thay đổi $P(X)$ mà không cần nhãn --- hướng phù hợp nhất với môi trường sản xuất công nghiệp thực tế.
...
Tuy nhiên, việc hiện thực hóa một hệ thống như vậy đối mặt với ba thách thức kỹ thuật trọng yếu. Thứ nhất là yêu cầu về độ trễ thấp trong phát hiện thời điểm trôi dạt xảy ra. Thứ hai là có khả năng phân loại các loại trôi dạt (sudden, gradual, incremental, recurrent) để xác định chiến lược thích ứng phù hợp cho mô hình. Cuối cùng là cơ chế cập nhật mô hình nhanh chóng mà không làm gián đoạn hoạt động của mô hình.

Từ những thách thức trên, đề tài này đặt ra những mục tiêu nghiên cứu và phát triển một hệ thống tự động phát hiện và thích ứng với hiện tượng trôi dạt khái niệm trong môi trường luồng dữ liệu thời gian thực, đặc biệt là trong môi trường không giám sát.
...
Lấy cảm hứng từ ý tưởng phân loại loại drift của CDT-MSW (Concept Drift Type identification based on Multi-Sliding Windows~\cite{guo2022cdtmsw}, sẽ được trình bày chi tiết ở Mục~\ref{sec:cdt-msw-theory}), luận văn đề xuất cách tiếp cận mới \textbf{SE-CDT} (ShapeDD-Enhanced Concept Drift Type Identification). Khác với CDT-MSW gốc yêu cầu nhãn để tính và tận dụng accuracy ratio để xác định và phân loại, SE-CDT thay thế hoàn toàn cơ chế này bằng các đặc trưng hình học từ tín hiệu MMD.
Phương pháp này có khả năng: (1) phát hiện chính xác thời điểm xảy ra \textit{drift} nhờ ShapeDD, và (2) sử dụng thông tin đó để trích xuất cửa sổ dữ liệu chính xác cho việc phân loại loại \textit{drift}, từ đó đưa ra chiến lược thích ứng phù hợp.
...
Thiết kế kiến trúc hệ thống trên môi trường dữ liệu streaming real-time bằng Apache Kafka. Thực hiện mô phỏng và đánh giá hiệu năng trong các kịch bản thử nghiệm để kiểm chứng tính khả thi của giải pháp đề xuất. (Lưu ý: Việc triển khai production thực tế và đánh giá hiệu năng chịu tải quy mô lớn là hướng phát triển tương lai).
...
còn gọi là \textit{covariate shift} hoặc \textit{data drift}. Cách tiếp cận này
dựa trên giả định rằng khi $P(X)$ thay đổi đáng kể, ranh giới quyết định $P(Y|X)$ cũng thường thay đổi theo (giả định joint drift). Việc phát hiện sớm sự thay đổi trong $P(X)$ đóng vai trò như tín hiệu cảnh báo, cho phép hệ thống kích hoạt các cơ chế kiểm tra hoặc cập nhật mô hình khi cần thiết.
...
\begin{itemize}
	...
	\item Đánh giá hiệu năng của hệ thống trên các tập dữ liệu tổng hợp đa dạng và các kịch bản drift khác nhau.
\end{itemize}
...
\begin{itemize}
	\item \textbf{Phương pháp phát hiện drift:} ShapeDD, ShapeDD-IDW (đề xuất), và các phương pháp
	      so sánh bao gồm MMD, KS-Test, D3, DAWIDD.
	\item \textbf{Mô hình phân loại drift:} SE-CDT --- phương pháp xác định
	      loại \textit{drift} dựa trên hình dạng tín hiệu MMD, lấy ý tưởng từ CDT-MSW.
	\item \textbf{Dữ liệu streaming:} Các tập dữ liệu tổng hợp đa dạng
	      (Gaussian, multimodal, high-dimensional,...) với các loại \textit{drift} khác nhau
	      (sudden, gradual, incremental, recurrent, blip).
	\item \textbf{Nền tảng xử lý streaming:} Apache Kafka cho việc triển khai
	      hệ thống giám sát \textit{real-time}.
\end{itemize}
