...
Dựa trên cơ sở lý thuyết và kết quả đánh giá hiệu năng, nghiên cứu sẽ tích hợp các kỹ thuật tiên tiến gần đây như ADW-MMD (Adaptive Density-Weighted MMD) và MMD-Agg (MMD Aggregated) vào framework ShapeDD. Mục tiêu là giải quyết các hạn chế về chi phí tính toán và độ nhạy của phương pháp gốc.

\subsection{Đề xuất phương pháp phân loại drift SHAPED\_CDT}
Kết hợp khả năng định vị chính xác thời điểm drift của ShapeDD với mô hình phân loại \textit{drift} CDT\_MSW~\cite{guo2022cdtmsw} để đề xuất phương pháp mới \textbf{SHAPED\_CDT} (ShapeDD-Enhanced Concept Drift Type Identification).
Hệ thống này có khả năng: (1) phát hiện chính xác thời điểm xảy ra \textit{drift} nhờ ShapeDD, và (2) sử dụng thông tin đó để trích xuất cửa sổ dữ liệu chính xác cho việc phân loại loại \textit{drift}, từ đó đưa ra chiến lược thích ứng tối ưu.
...
\textbf{đa dạng các loại drift} bao gồm sudden, gradual, incremental, recurrent, và blip.
Cách tiếp cận đánh giá bao gồm:

\begin{enumerate}
	\item \textbf{Đánh giá toàn diện:} Thực nghiệm trên 5 kịch bản drift khác nhau
	      với 10 runs mỗi kịch bản để đảm bảo độ tin cậy thống kê.

	\item \textbf{Phân tích đa tầng:} Đánh giá cả khả năng \textit{phát hiện} (Detection Rate)
	      lẫn khả năng \textit{phân loại} (Classification Accuracy) của hệ thống,
	      bao gồm cả phân loại Category (TCD vs PCD) và Subcategory (Sudden, Gradual, Incremental, Recurrent, Blip).

	\item \textbf{So sánh công bằng:} Đánh giá phương pháp đề xuất (SE-CDT không giám sát)
	      so với baseline CDT\_MSW có giám sát trong cùng điều kiện thử nghiệm,
	      bao gồm cả setup supervised với concept-aware labels.
\end{enumerate}
