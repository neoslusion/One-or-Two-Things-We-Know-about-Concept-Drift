...
Trong đề tài này, tác giả giải quyết các thách thức trên bằng cách xây dựng một hệ thống end-to-end với bốn thành phần chính:
\begin{enumerate}
    \item Phát hiện drift không giám sát sử dụng phương pháp ShapeDD - một phương pháp phát hiện trôi dạt;
    \item Phân loại loại drift tự động sử dụng CDT\_MSW;
    \item Chiến lược thích ứng (adaptation strategies) đa dạng cho từng loại drift;
    \item Kiến trúc streaming real-time sử dụng Apache Kafka cho việc xử lý event-driven có khả năng mở rộng.
\end{enumerate}
Điều này đòi hỏi sự kết hợp giữa việc nhanh chóng phát hiện và phân loại drift, cùng với việc áp dụng các chiến lược thích ứng mô hình phù hợp để duy trì hiệu suất của hệ thống trong môi trường dữ liệu streaming động.
...
\textbf{Trọng tâm đánh giá:} Mặc dù hệ thống được thiết kế để hỗ trợ nhiều loại drift (sudden, gradual, incremental, recurrent, blip), luận văn này \textbf{tập trung đánh giá chuyên sâu vào sudden drift (trôi dạt đột ngột)}. Lý do cho sự lựa chọn này bao gồm: (1) sudden drift có tầm quan trọng thực tế cao nhất trong các hệ thống production, (2) ShapeDD có nền tảng lý thuyết vững chắc nhất cho abrupt drift (Triangle Shape Property), và (3) để đảm bảo chất lượng nghiên cứu với phân tích sâu thay vì coverage nông. Các loại drift khác đã được implement trong framework và có thể được đánh giá trong nghiên cứu tương lai.
