...
một cửa sổ dữ liệu với độ dài biến thiên sao cho giả thiết "không có sự thay đổi" được đảm bảo bên trong cửa sổ đó. Cụ thể,
tại mỗi thời điểm, ADWIN tách cửa sổ hiện tại thành hai phần $W_0$ và $W_1$ liên tiếp rồi so sánh
...
Ưu điểm nổi bật của ADWIN là nó cung cấp một sự đảm bảo về mặt lý thuyết cho độ chính xác của phát hiện thay đổi, đồng thời tự động
...
phí tính toán khá cao do phải kiểm tra nhiều kích thước cửa sổ, và nhạy cảm với nhiễu (noise) trong dữ liệu~\cite{hinder2024survey_partA}.
...
\textbf{HDDM (Hoeffding's Drift Detection Method)}~\cite{HDDM} là một hướng cải tiến quan trọng, áp dụng các công cụ thống kê chặt chẽ hơn để giảm
...
Một hướng khác là tận dụng \textbf{ensemble learning}, dùng nhiều mô hình phân loại chạy song song, không chỉ để nâng cao độ chính xác mà còn để nhận biết concept drift thông qua sự đa dạng (diversity) của ensemble. Ví dụ, phương pháp \textbf{ACE} (Adaptive Classifier Ensemble~\cite{nishida2007ace}, 2007) duy trì một tập phân loại và liên tục đánh giá mỗi mô hình trên luồng dữ liệu. Khi có mô hình suy giảm độ chính xác, nó được thay thế bởi mô hình mới huấn luyện trên dữ liệu gần đây. Việc mô hình cũ bị loại bỏ chính là dấu hiệu của drift (concept cũ không còn phù hợp). Tương tự, \textbf{Learn++.NSE}~\cite{elwell2011learn} mở rộng ý tưởng ensemble bằng cách gán trọng số thời gian (temporal weights) cho các classifier theo độ tuổi: classifier mới được thêm vào để học concept mới còn classifier cũ mờ dần. Các phương pháp ensemble phát hiện drift một cách gián tiếp thông qua cơ chế tự điều chỉnh, khi drift được phản ánh qua thay đổi cấu trúc hoặc thành phần của ensemble.
...
mô hình phát hiện bất thường dùng LSTM, các nghiên cứu trước đây cũng nhúng các detector như DDM hoặc Page-Hinkley để liên tục canh chừng lỗi dự đoán theo thời gian.
...
Sau khi xác định được vị trí trôi dạt $t$, thuật toán tiếp tục \textbf{mở rộng vùng quan sát} để ước lượng \textbf{độ dài trôi dạt} (drift length, $m$). Độ dài trôi dạt ($m$) là khoảng thời gian dữ liệu cần để thay đổi hoàn toàn từ một phân phối ổn định cũ sang một phân phối ổn định mới [5].

Trong giai đoạn này, một \textbf{cửa sổ phụ} (Adjoint window - $W_R$) được kích hoạt ngay sau vị trí $t$. Cửa sổ $W_R$ này có kích thước cố định bao gồm $n$ cửa sổ cơ bản [3]. Thuật toán sẽ phát hiện độ dài $m$ dựa trên việc đo lường \textbf{phương sai} của độ chính xác thời gian thực ($r^2$) bên trong $W_R$:
\begin{equation}
...
\begin{itemize}
    ...
    \item $\overline{a}_{R}^{t+i}$ là độ chính xác trung bình của $n$ cửa sổ cơ bản trong $W_R$ ở vị trí hiện tại [1].
\end{itemize}

Việc định danh danh mục (category) dựa trên mức độ ổn định của phân phối dữ liệu được đánh giá qua ngưỡng $d$ (drift length detection parameter) [2]:
\begin{itemize}
...
Quá trình trượt kết thúc sau $k$ bước ($k$ là tham số dừng theo dõi) [6]. Tập hợp các giá trị $\tilde{P}^{i}_{\text{tra}}$ tạo thành một \textbf{Đường cong TFR} (TFR curve). Hình dáng của đường cong này qua 4 giai đoạn (tương ứng với vị trí tương đối của $W_A^{\prime}$ và $W_B^{\prime}$) được dùng để xác định loại trôi dạt:
...
Luận văn vì vậy tập trung vào việc \textbf{cải tiến ShapeDD} theo hai hướng bổ sung cho nhau: (1) thay permutation test bằng IDW-MMD kết hợp asymptotic p-value để tăng tốc độ xử lý, và (2) xây dựng SE-CDT để phân loại loại drift dựa trên hình dạng tín hiệu MMD mà không cần nhãn. Chi tiết được trình bày trong Chương~\ref{chap:theoretical-foundation} và Chương~\ref{chap:proposed-model}.
