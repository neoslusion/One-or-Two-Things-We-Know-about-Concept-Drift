...
Gần đây, Schrab et al. (2023)~\cite{schrab2023mmdagg} đề xuất \textbf{MMD-Agg} (MMD Aggregated), một phương pháp sử dụng multiple kernel bandwidths để tăng robustness trong phát hiện thay đổi phân phối. Phương pháp này có đảm bảo lý thuyết mạnh về statistical power và Type I error control, tuy nhiên chi phí tính toán cao ($\sim$500ms/window với wild bootstrap) khiến không phù hợp với streaming context yêu cầu low latency.
...
\subsection{Giai đoạn 1: Phát hiện trôi dạt (Detection Process)}
Trong giai đoạn đầu, CDT-MSW định nghĩa \textbf{Tỷ lệ Phát hiện Luồng} (Detection Flow Ratio) để đo lường sự thay đổi giữa hai cửa sổ dữ liệu $W_A$ (tĩnh/tham chiếu) và $W_B$ (trượt/hiện tại):
\begin{equation}
    \tilde{P}^{t}_{\text{det}} = \frac{\alpha^t_{B}}{\alpha^{\star}_{A}}
    \label{eq:dfr}
\end{equation}
Trong đó $\alpha^{\star}_{A}$ là độ chính xác thời gian thực (real-time accuracy) của mô hình huấn luyện trên cửa sổ $W_A$, và $\alpha^t_{B}$ là độ chính xác trên cửa sổ $W_B$ sau khi trượt $t$ lần.
\begin{itemize}
    \item Khi $\tilde{P}^{t}_{\text{det}} \approx 1$: Độ chính xác trên hai cửa sổ tương đương, không có drift.
    \item Khi $\tilde{P}^{t}_{\text{det}} < \tau$ (ngưỡng phát hiện): Độ chính xác trên $W_B$ suy giảm đáng kể so với $W_A$, điều đó thể hiện có khả năng xảy ra drift.
\end{itemize}
Khi $\tilde{P}^{t}_{\text{det}}$ giảm mạnh (thỏa mãn $\tilde{P}^{t}_{\text{det}} < \tau$), thuật toán xác định đây là vị trí có khả năng xảy ra trôi dạt, ký hiệu là $t_d$.

\subsection{Giai đoạn 2: Đo độ dài trôi dạt và Nhận dạng Danh mục (Growth Process)}
Sau khi xác định được vị trí $t_d$, thuật toán tiếp tục \textbf{mở rộng vùng quan sát} để ước lượng \textbf{độ dài trôi dạt} (drift length, $m$). Độ dài trôi ($m$) là khoảng thời gian dữ liệu cần để thay đổi hoàn toàn từ một phân phối ổn định sang một phân phối ổn định mới.

Trong giai đoạn này, một \textbf{cửa sổ phụ cận} ($W_R$) được kích hoạt để phát hiện độ dài $m$ dựa trên \textbf{phương sai} của độ chính xác thời gian thực ($\sigma_R^{2}$) trong $W_R$:
\begin{equation}
    \sigma_{R}^{2\;t+i} = \frac{\sum_{j=0}^{n-1}(\alpha_{R_{j}}^{t+i+j}-\overline{\alpha}_{R}^{t+i})^{2}}{n}.
\end{equation}
Trong đó, $\overline{\alpha}_{R}^{t+i}$ là độ chính xác trung bình của các cửa sổ cơ bản ($W_{R_j}$) trong $W_R$ sau khi trượt $i-1$ bước.

Việc phát hiện độ dài dựa trên sự ổn định của phân phối dữ liệu trong $W_R$:
\begin{itemize}
    \item Nếu $\sigma_{R}^{2\;t+1} \le \delta$ (ngưỡng phát hiện độ dài): Độ dài trôi $m=1$. Danh mục là \textbf{Trôi khái niệm Tức thời} (Transient Concept Drift - TCD).
    \item Nếu $\sigma_{R}^{2\;t+i} > \delta$: $W_R$ tiếp tục trượt. Quá trình kết thúc khi $\sigma_{R}^{2\;t+m} \le \delta$, độ dài trôi $m > 1$. Danh mục là \textbf{Trôi khái niệm Tiệm tiến} (Progressive Concept Drift - PCD).
\end{itemize}
Trong quá trình này, các cửa sổ cơ bản $W_A$ và $W_B$ dần phát triển thành các \textbf{cửa sổ tổng hợp} $W_A^{\prime}$ và $W_B^{\prime}$ với kích thước bằng $m$ lần kích thước cửa sổ cơ bản ($ms$).

\subsubsection*{Đánh giá độ chính xác ước lượng vùng Drift}
Độ chính xác của việc ước lượng vùng drift được đánh giá bằng các chỉ số:
\begin{itemize}
    \item \textbf{LLR (Length Loss Rate):} Tỷ lệ phần vùng drift bị mất (ước lượng thiếu). Mục tiêu: LLR càng thấp càng tốt.
    \item \textbf{LOR (Length Overflow Rate):} Đo lường mức độ \emph{ước lượng quá mức} (overestimation) của độ dài vùng drift. Mục tiêu: LOR càng thấp càng tốt (lý tưởng = 0).
    \item \textbf{LRR (Length Recall Rate):} Đo lường khả năng \emph{phát hiện đầy đủ} vùng drift ($LRR = 1 - LLR$). Mục tiêu: LRR càng cao càng tốt (lý tưởng = 1.0).
\end{itemize}

\subsection{Giai đoạn 3: Theo dõi và nhận dạng loại drift con (Tracking Process)}
Để phân biệt chi tiết giữa các loại trôi dạt, CDT-MSW định nghĩa \textbf{Tỷ lệ Theo dõi Luồng} (Tracking Flow Ratio – TFR) bằng cách cho $W_A^{\prime}$ trượt về phía trước để theo dõi $W_B^{\prime}$ (tĩnh):
\begin{equation}
    \tilde{P}^{i}_{\text{tra}} = \frac{\alpha^{\star}_{B^{\prime}}}{\alpha^{i}_{A^{\prime}}}
    \label{eq:tfr}
\end{equation}
Trong đó $\alpha^{\star}_{B^{\prime}}$ là độ chính xác của mô hình trên cửa sổ $W'_B$ tại vị trí ban đầu ($t$), và $\alpha^{i}_{A^{\prime}}$ là độ chính xác trên cửa sổ $W'_A$ khi trượt đến vị trí $i$.

Dãy $\tilde{P}^{i}_{\text{tra}}$ được tổng hợp thành \textbf{Đường cong TFR} (TFR curve), thể hiện hình dạng và tốc độ thay đổi của khái niệm. Đường cong này dùng để phân biệt các loại drift cụ thể:

\subsubsection*{Phân biệt TCD (Sudden, Blip, Recurrent)}
\begin{itemize}
    \item \textbf{Sudden drift:} $\tilde{P}_{\text{tra}}$ nhỏ hơn 1 ở giai đoạn đầu, sau đó tăng lên gần 1 và duy trì ổn định.
    \item \textbf{Blip:} TFR tương tự Sudden ở hai giai đoạn đầu, nhưng sau đó giảm đột ngột xuống dưới 1 và duy trì (do dữ liệu trong $W_A^{\prime}$ khôi phục lại phân phối cũ).
    \item \textbf{Recurrent drift:} $\tilde{P}_{\text{tra}}$ giảm xuống dưới 1 sau khi tăng lên gần 1 trong một khoảng thời gian (do bản chất là sự kết hợp của hai Sudden).
\end{itemize}

\subsubsection*{Phân biệt PCD (Incremental và Gradual) bằng MTFR}
Đối với PCD (Incremental và Gradual), do cửa sổ tổng hợp $W_A^{\prime}$ và $W_B^{\prime}$ lớn, làm giảm sự khác biệt giữa hai loại này, một đường cong \textbf{Micro TFR (MTFR)} được tính toán để \emph{khuếch đại sự khác biệt}.
\begin{itemize}
    \item \textbf{Incremental/Gradual:} Giá trị $\tilde{P}_{\text{tra}}$ chuyển tiếp chậm từ nhỏ hơn 1 sang lớn hơn 1.
    \item \textbf{MTFR} được tính bằng tỷ lệ độ chính xác kiểm tra của cửa sổ cơ bản đầu tiên trong $W_B^{\prime}$ so với cửa sổ cơ bản đầu tiên trong $W_A^{\prime}$ trong suốt quá trình theo dõi.
\end{itemize}

\subsection{Đánh giá khả năng nhận dạng loại trôi dạt}
Khả năng phân loại chính xác các loại trôi dạt được đánh giá bằng hai chỉ số:
\begin{itemize}
    \item \textbf{ACC$_{\text{cat}}$} (Drift category accuracy): Độ chính xác nhận dạng loại chính (TCD/PCD).
    \begin{equation}
        \text{ACC}_{\text{cat}} = \frac{T_{T} + T_{P}}{T_{T} + F_{T} + T_{P} + F_{P}}
        \label{eq:acc-cat}
    \end{equation}
    Trong đó $T_{T}$ và $F_{T}$ là số lượng nhận dạng đúng/sai cho TCD; $T_{P}$ và $F_{P}$ là số lượng nhận dạng đúng/sai cho PCD.
    
    \item \textbf{ACC$_{\text{subcat}}$} (Drift subcategory accuracy): Độ chính xác nhận dạng loại phụ (Sudden, Gradual, Incremental, Recurrent).
    \begin{equation}
        \text{ACC}_{\text{subcat}} = \frac{\sum_{i}T_{T}^{i} + \sum_{j}T_{P}^{j}}{T_{T} + T_{P}}
        \label{eq:acc-subcat}
    \end{equation}
    Trong đó $i \in \{\text{Sudden, Blip, Recurrent}\}$ và $j \in \{\text{Incremental, Gradual}\}$.
\end{itemize}
