...
Chẳng hạn, kiểm định PH \cite{inproceedings} được áp dụng trong bối cảnh concept drift để giám sát trung bình của một chuỗi 
...
Một phương pháp khác là \textbf{KSWIN} (Kolmogorov–Smirnov WINdowing \cite{inbook}), kết hợp kiểm định Kolmogorov–Smirnov với cửa 
...
\textbf{EDDM} cải thiện khả năng phát hiện gradual drift bằng cách thay đổi metric theo dõi. Thay vì theo dõi trực tiếp tỷ lệ lỗi, EDDM tập trung 
...
\textbf{FHDDM (Fast HDDM, 2016)} được giới thiệu như phiên bản tăng tốc của HDDM, cải thiện cả tốc độ và độ nhạy trong việc phát hiện drift. 
...
\textbf{MDDM (McDiarmid's Drift Detection Method)} là phương pháp áp dụng bất đẳng thức McDiarmid – một bất đẳng thức tập trung (concentration inequality) 
...
\textbf{RDDM (Reactive DDM, 2017)} tập trung vào việc tối ưu thời gian phản ứng bằng cách rút ngắn giai đoạn cảnh báo (warning phase) của DDM gốc. 
...
Cải tiến này đặc biệt hữu ích trong các ứng dụng yêu cầu phản ứng thời gian thực. Những năm gần đây còn xuất hiện các biến thể hướng đến các tình 
huống đặc thù: \textbf{ACDDM} (Accurate DDM, 2020) cải thiện độ chính xác cho cả drift lặp lại bằng cách ghi nhớ các concept 
cũ, \textbf{DMDDM} (Diversity-Measure DDM, 2020) tận dụng sự đa dạng (diversity) của một tập classifier để nhận biết drift mạnh 
hơn và \textbf{DDM-FPW} (2020) kiểm soát tỷ lệ dương tính giả (false positive rate) trong luồng dữ liệu IoT đa nhãn (multi-label streams). 
Mục tiêu chung của các cải tiến này là tăng độ nhạy với nhiều loại drift (sudden, gradual, incremental, recurrent), đồng thời giảm báo động giả và 
phân biệt được drift thực sự với nhiễu.
...
\textbf{Paired Learners} (Bach \& Maloof, 2008) là một ví dụ tiêu biểu, sử dụng hai mô hình học song song trên dòng dữ liệu: một mô 
...
Một hướng khác là tận dụng \textit{ensemble learning} – sử dụng nhiều mô hình phân loại chạy song song – không chỉ để nâng cao độ 
chính xác, mà còn để nhận biết concept drift thông qua sự đa dạng (diversity) của ensemble. Ví dụ, 
phương pháp \textbf{ACE} (Adaptive Classifier Ensemble, 2009) duy trì một tập phân loại viên và liên tục đánh giá mỗi mô hình trên luồng dữ 
...
chính là dấu hiệu của drift (tức concept cũ không còn phù hợp). Tương tự, \textbf{Learn++.NSE} (Elwell \& Polikar, 2011) mở rộng ý tưởng 
...
Như bảng \ref{tab:drift-history} tóm tắt, lĩnh vực phát hiện trôi dạt khái niệm đã phát triển phong phú qua thời gian. Từ những ý tưởng sơ 
