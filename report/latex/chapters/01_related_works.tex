...
Trong giai đoạn này, một \textbf{cửa sổ phụ cận} ($W_R$) được kích hoạt để phát hiện độ dài $m$ dựa trên \textbf{phương sai} của độ chính xác thời gian thực ($\sigma^2_R$) trong $W_R$:
\begin{equation}
    \sigma_{R}^{t+i^{2}} = \frac{\sum_{j=0}^{n-1}(\alpha_{R_{j}}^{t+i+j}-\overline{\alpha}_{R}^{t+i})^{2}}{n}.
    \tag{3}
\end{equation}
...
\begin{itemize}
    \item Nếu $\sigma_{R}^{t+1^{2}} \le \delta$ (ngưỡng phát hiện độ dài): Độ dài trôi $m=1$. Danh mục là \textbf{Trôi khái niệm Tức thời} (Transient Concept Drift - TCD).
    \item Nếu $\sigma_{R}^{t+i^{2}} > \delta$: $W_R$ tiếp tục trượt. Quá trình kết thúc khi $\sigma_{R}^{t+m^{2}} \le \delta$, độ dài trôi $m > 1$. Danh mục là \textbf{Trôi khái niệm Tiệm tiến} (Progressive Concept Drift - PCD).
\end{itemize}
...
Tóm lại, CDT-MSW nhận dạng loại trôi dạt dựa trên hai chỉ số động: \textbf{Detection Flow Ratio} (Công thức~1)
