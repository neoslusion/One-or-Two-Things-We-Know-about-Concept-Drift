...
Sự thay đổi bắt đầu diễn ra vào giữa những năm 2000 khi các thuật toán chuyên biệt để phát hiện điểm thay đổi bắt đầu ra đời.
...
thống (ví dụ sử dụng ensemble hoặc mô hình phụ để nhận biết drift), và (4) nhóm phương pháp học sâu (deep learning).
...
phương pháp \textbf{ACE} (Adaptive Classifier Ensemble~\cite{nishida2007ace}, 2007) duy trì một tập phân loại và liên tục đánh giá mỗi mô hình trên luồng dữ
...
chỉnh, đó là khi drift được phản ánh khi cấu trúc hoặc thành phần của ensemble thay đổi.
...
Tổng kết lại, xuyên suốt quá trình hình thành và phát triển, đã có sự gia tăng đáng kể các công trình kết hợp giữa deep learning và phát hiện
...
Sau khi phát hiện được thời điểm xảy ra trôi dạt, việc xác định loại trôi dạt là điều cần thiết để có thể có những hành động phù hợp để can thiệp vào mô hình. Khác với các phương pháp truyền thống chỉ xác định có trôi dạt hay không, phương pháp \textbf{CDT-MSW} (Concept Drift Type identification based on Multi-Sliding Windows) được Guo \textit{et al.}~ \cite{guo2022cdtmsw} đề xuất nhằm \emph{phân biệt bản chất và tốc độ thay đổi của quá trình drift}. Phương pháp này dựa trên việc quan sát hành vi thay đổi của độ chính xác giữa nhiều cửa sổ trượt (traditional sliding windows và adaptive windows), qua đó nhận dạng các loại drift như: \emph{sudden}, \emph{gradual}, \emph{incremental}, và \emph{recurrent}.
...
\begin{enumerate}
	...
	\item Các chỉ số định lượng như độ dài trôi dạt $L_d$ và tốc độ thay đổi $r_t$ của CDT-MSW
	      có thể được dùng để điều chỉnh động learning rate hoặc quyết định chiến lược chuyển mô hình (switching strategy).
\end{enumerate}
Tóm lại, chiến lược cập nhật mô hình thích ứng đóng vai trò cầu nối giữa phát hiện trôi dạt và duy trì hiệu năng hệ thống. Các phương pháp hiện đại kết hợp cả thông tin thống kê từ bộ phát hiện trôi dạt như CDT-MSW và đặc trưng động của mô hình học sâu, cho phép \textbf{thích ứng chính xác theo từng loại trôi dạt}, giảm thiểu hiện tượng quên kiến thức cũ và cải thiện độ bền vững của hệ thống học trực tuyến.

\section{Lý do lựa chọn ShapeDD làm nền tảng}
\label{sec:why-shapedd}

Từ khảo sát các phương pháp trên, có thể rút ra nhận xét tổng hợp để định hướng cho phần đề xuất của luận văn.

Các phương pháp theo dõi hiệu năng mô hình (DDM, EDDM, ADWIN...) hoạt động tốt nhưng \textbf{đòi hỏi nhãn dữ liệu} ($y$) để đo lỗi --- điều không khả thi trong môi trường sản xuất công nghiệp mà luận văn nhắm đến. Các phương pháp thống kê thuần túy như KS-Test có thể hoạt động không cần nhãn, nhưng không xác định được \textit{vị trí chính xác} của điểm drift và có tỉ lệ báo động giả cao. Các phương pháp kernel-based như MMD chuẩn và DAWIDD tiệm cận hơn với yêu cầu, nhưng chi phí tính toán lớn (2500 permutation test mỗi lần đánh giá) và không tích hợp sẵn cơ chế phân loại loại drift.

\textbf{ShapeDD} nổi trội ở ba điểm phù hợp trực tiếp với mục tiêu của luận văn: (1) hoạt động hoàn toàn \textit{không giám sát} (không cần nhãn), (2) xác định được \textit{vị trí chính xác} của điểm drift nhờ đặc trưng hình học tam giác (Triangle Shape Property), và (3) nền tảng lý thuyết rõ ràng cho phép cải tiến có chủ đích. Tuy nhiên, ShapeDD gốc vẫn phụ thuộc vào permutation test tốn kém và chưa có cơ chế phân loại loại drift.

Luận văn này vì vậy tập trung vào việc \textbf{cải tiến ShapeDD} theo hai hướng bổ sung cho nhau: (1) thay thế permutation test bằng IDW-MMD kết hợp asymptotic p-value để tăng tốc độ xử lý, và (2) xây dựng phương pháp SE-CDT để phân loại loại drift dựa trên hình dạng tín hiệu MMD mà không cần nhãn. Chi tiết được trình bày trong Chương~\ref{chap:theoretical-foundation} và Chương~\ref{chap:proposed-model}.
