...
Shape Drift Detector (ShapeDD)~\cite{hinder2024survey_partA} là detector dựa trên Maximum Mean Discrepancy (MMD)~\cite{gretton2012kernel} hoạt động qua bốn giai đoạn:

\begin{enumerate}
    \item \textbf{Thu thập dữ liệu:} Sử dụng cửa sổ trượt $W_t$ kích thước $2l_1$ trên luồng dữ liệu
    \item \textbf{Tính MMD:} So sánh hai nửa cửa sổ bằng MMD với kernel Gaussian RBF
    \item \textbf{Shape detection:} Phát hiện hình tam giác đặc trưng của drift qua convolution filtering
    \item \textbf{Xác thực:} Sử dụng permutation test~\cite{good2005permutation} để xác nhận drift có ý nghĩa thống kê ($p < \alpha$)
\end{enumerate}

Ý tưởng cốt lõi: Khi drift xảy ra, tín hiệu MMD tạo hình tam giác (tăng rồi giảm) do cửa sổ trượt qua vùng thay đổi. Shape filtering giúp giảm false alarm đáng kể so với methods chỉ dựa vào ngưỡng MMD đơn thuần. Chi tiết đầy đủ về cơ chế hoạt động được trình bày trong Chapter 2 (Nền tảng lý thuyết).
...
Họ phương pháp DDM (Drift Detection Method) được phát triển từ năm 2004~\cite{gama2004learning} đến nay, trở thành nền tảng quan trọng của drift detection.
