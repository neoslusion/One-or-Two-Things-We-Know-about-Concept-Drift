...
\begin{table}[!htbp]
	\centering
	\caption{So sánh hiệu năng của một số phương pháp phát hiện drift}
...
\begin{itemize}
    \item Nếu $r_{t+1,R}^2 \le d$ (ổn định ngay lập tức): Độ dài trôi $m=1$. Danh mục được phân loại là \textbf{Trôi dạt Khái niệm Tức thời} (Transient Concept Drift - TCD) [2].
    \item Nếu $r_{t+i,R}^2 > d$ (chưa ổn định): $W_R$ tiếp tục trượt về phía trước với bước trượt là $s$. Quá trình kết thúc khi đạt trạng thái $r_{t+m,R}^2 \le d$. Lúc này độ dài trôi $m > 1$, và danh mục được phân loại là \textbf{Trôi dạt Khái niệm Tiệm tiến} (Progressive Concept Drift - PCD) [2].
\end{itemize}

Đồng thời trong quá trình $W_R$ trượt đi, các cửa sổ cơ bản $W_A$ và $W_B$ dần dần phát triển (grow) thành các \textbf{cửa sổ tổng hợp} (composite windows) $W_A^{\prime}$ và $W_B^{\prime}$. Kích thước cuối cùng của chúng bằng $m$ lần kích thước cửa sổ cơ bản ($ms$) [2].

\subsubsection*{Đánh giá độ chính xác ước lượng vùng Drift đối với nhóm PCD}
Khả năng ước lượng độ dài trôi dạt của thuật toán được đánh giá qua 3 chỉ số [6], [7], [8], [4]:
\begin{itemize}
...
Quá trình trượt kết thúc sau $k$ bước ($k$ là tham số dừng theo dõi) [6]. Tập hợp các giá trị $\tilde{P}^{i}_{\text{tra}}$ tạo thành một \textbf{Đường cong TFR} (TFR curve). Hình dáng của đường cong này qua 4 giai đoạn (tương ứng với vị trí tương đối của $W_A^{\prime}$ và $W_B^{\prime}$) được dùng để định danh tiểu loại trôi dạt [2]:

\subsubsection*{Phân biệt nhóm TCD (Sudden, Blip, Recurrent)}
Đối với nhóm TCD, đường cong TFR thể hiện các đặc trưng rõ rệt [3]:
\begin{itemize}
...
Đối với nhóm PCD, vì kích thước cửa sổ tổng hợp $W_A^{\prime}$ và $W_B^{\prime}$ lớn (bằng $m \times s$), sự khác biệt tín hiệu giữa Incremental và Gradual bị pha loãng [4]. Do đó, thuật toán tính toán thêm một đường cong \textbf{Micro TFR (MTFR)} để khuếch đại sự khác biệt này:
\begin{itemize}
    \item \textbf{MTFR} được tính bằng tỷ lệ độ chính xác kiểm thử (testing accuracy) của riêng \emph{cửa sổ cơ bản đầu tiên} nằm trong $W_B^{\prime}$ so với \emph{cửa sổ cơ bản đầu tiên} nằm trong $W_A^{\prime}$ trong suốt quá trình theo dõi [4, 5].
    \item \textbf{Đặc trưng Incremental/Gradual:} Thay vì biến đổi sắc nét như TCD, đường cong giá trị của nhóm này sẽ chuyển tiếp từ từ (slowly transition) từ mức nhỏ hơn 1 sang lớn hơn 1 dựa trên sự thay đổi phân phối tiệm tiến của chúng [7]. Dựa vào sự biến thiên của MTFR ở các giai đoạn giữa, thuật toán sẽ tách biệt được Incremental và Gradual.
\end{itemize}

\subsection{Đánh giá khả năng nhận dạng loại trôi dạt}
Khả năng phân loại chính xác các loại trôi dạt của hệ thống được đánh giá khách quan bằng hai chỉ số [8, 9]:
\begin{itemize}
