...
\begin{itemize}
	...
	\item \textbf{SE-CDT và ShapeDD\_WMMD\_PROPER (F1 = 0.481):} Hai phương pháp đề xuất đạt Precision cao (0.539) với \textbf{không có báo động giả} (FP = 0) trên benchmark detection. Đây là ưu điểm vượt trội so với các phương pháp giám sát (như DDM/ADWIN) thường báo động sai khi gặp Virtual Drift (thay đổi nhãn nhưng $P(X)$ giữ nguyên). Việc SE-CDT duy trì FP=0 trên các tập dữ liệu nhiễu nhãn (như \textit{standard\_sea}) chứng minh tính bền vững (robustness) của phương pháp unsupervised trong các hệ thống thực tế, giúp giảm thiểu hiện tượng "alert fatigue" cho người vận hành.
...
Để có cái nhìn khách quan, chúng tôi đối chiếu SE-CDT với CDT\_MSW~\cite{guo2022cdtmsw} --- một phương pháp supervised tiêu biểu. Chúng tôi thực hiện so sánh kép: (1) so với kết quả tự thực nghiệm (local baseline) và (2) so với các giá trị được công bố trong bài báo gốc (paper values).

Bảng~\ref{tab:cdt-comparison-main} so sánh SE-CDT với kết quả của CDT\_MSW.

\begin{table}[H]
	\centering
	\caption{So sánh SE-CDT với CDT\_MSW (Tổng hợp)}
...
\textbf{Nhận xét:}
\begin{itemize}
    \item \textbf{Vượt trội so với local baseline:} Trong cùng điều kiện thử nghiệm trên bộ benchmark đa dạng, SE-CDT (81.2\% CAT) vượt xa phiên bản thực nghiệm của CDT\_MSW (53.2\% CAT). Điều này cho thấy tính ổn định của phương pháp unsupervised khi đối mặt với nhiều kịch bản drift phức tạp.
    \item \textbf{So với paper values:} Mặc dù độ chính xác CAT của SE-CDT (81.2\%) thấp hơn nhẹ so với giá trị báo cáo trong bài báo gốc của CDT\_MSW (87.5\%), SE-CDT lại vượt trội về độ chính xác phân loại chi tiết (SUB Accuracy 50\% so với 42\%) và khả năng phát hiện sự kiện (EDR 94.4\% so với 80\%).
    \item \textbf{Độ trễ và Tính thực tiễn:} Như minh họa trong Hình~\ref{fig:detection_timeline_mixed_a}, SE-CDT phát hiện drift sớm hơn đáng kể nhờ việc trích xuất trực tiếp tín hiệu từ $P(X)$, thay vì chờ đợi sự suy giảm hiệu suất (accuracy drop) như các phương pháp supervised.
\end{itemize}

\subsection{Phân tích chi tiết theo loại drift}
\label{sec:cdt-per-type}

Để hiểu sâu hơn về khả năng phân loại của SE-CDT, Bảng~\ref{tab:cdt-per-type} trình bày hiệu suất trên từng loại drift cụ thể so với paper values của CDT\_MSW.

\begin{table}[H]
	\centering
	\caption{So sánh hiệu suất phân loại theo loại drift (so với paper values)}
...
\textbf{Phân tích kết quả:}
\begin{itemize}
	\item \textbf{Sudden Drift (88.2\%):} Điểm mạnh chính của SE-CDT. MMD signal tạo peak rõ ràng tại thời điểm thay đổi đột ngột, dễ dàng phân biệt.
	
	\item \textbf{Incremental Drift (40.0\%):} Cải thiện vượt bậc so với baseline (8.5\%) nhờ các đặc trưng temporal (LTS, SDS, MS) giúp nhận diện xu hướng thay đổi liên tục.
	
	\item \textbf{Recurrent Drift (0\%):} Hạn chế hiện tại --- hệ thống xử lý các concept lặp lại như các sự kiện độc lập do thiếu cơ chế bộ nhớ dài hạn (concept memory).
\end{itemize}
...
Để khắc phục hạn chế của các bộ tạo dữ liệu Sudden Drift truyền thống (thường luân phiên giữa hai concept A $\to$ B $\to$ A, dẫn đến việc mô hình tĩnh có độ chính xác ảo ~75\%), chúng tôi đã bổ sung kịch bản **Stepping Drift**. Đây là dạng trôi dạt tích lũy (Cumulative Sudden Drift), nơi phân phối dữ liệu liên tục dịch chuyển ra xa trạng thái ban đầu ($P_0 \to P_1 \to P_2 \dots$), buộc mô hình phải học lại liên tục để duy trì hiệu suất.
...
Để đối chiếu, chúng tôi cũng trình bày kết quả trên kịch bản Sudden Drift tiêu chuẩn (luân phiên A$\to$B$\to$A). Trong trường hợp này, mô hình tĩnh ("No Adaptation") vẫn duy trì độ chính xác trung bình khá cao ($\sim 75\%$) do concept A lặp lại. Tuy nhiên, điều này chỉ che giấu vấn đề: mô hình thực tế đã thất bại hoàn toàn trong các giai đoạn concept B. Hệ thống đề xuất (Type-Specific) đảm bảo độ chính xác ổn định >98% trong \textit{mọi} giai đoạn, bất kể concept có lặp lại hay không.
