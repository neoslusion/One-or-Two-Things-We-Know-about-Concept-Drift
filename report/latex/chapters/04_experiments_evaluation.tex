...
% =============================================================================
\input{../../experiments/benchmark/publication_figures/table_I_comprehensive_performance.tex}

\textbf{Phân tích kết quả:}
\begin{itemize}
	\item \textbf{MMD\_IDW dẫn đầu về F1 ($0.576 \pm 0.442$):} Phương pháp sử dụng trọng số nghịch mật độ (Inverse Density-Weighted MMD) đạt kết quả tốt nhất, cân bằng tốt giữa Precision (0.627) và Recall (0.578).
	\item \textbf{ShapeDD\_WMMD\_PROPER xếp hạng 2 (F1 = $0.562 \pm 0.436$):} Biến thể kết hợp IDW-MMD với đặc trưng hình dạng (shape feature) và asymptotic p-value duy trì hiệu suất cao và có độ trễ phát hiện thấp (Mean Time To Detection = 20 mẫu), cải thiện đáng kể so với ShapeDD gốc (MTTD = 31 mẫu).
...
\begin{figure}[h]
	...
	\caption[Biểu đồ Critical Difference (CD)]{Biểu đồ Critical Difference (CD) với mức ý nghĩa $\alpha = 0.05$. MMD\_ADW và ShapeDD\_ADW\_MMD nằm ở nhóm đầu (rank thấp nhất là tốt nhất).}
	\label{fig:critical_difference}
\end{figure}

Mặc dù p-value tổng thể của kiểm định Friedman ($p \approx 0.8$) cho thấy sự khác biệt giữa các nhóm chưa đạt mức ý nghĩa thống kê cao (do phương sai lớn giữa các dataset), thứ hạng trung bình (Rank) của MMD\_ADW (1.0) và ShapeDD\_ADW\_MMD (2.0) ổn định hơn hẳn so với các phương pháp khác.
...
\begin{enumerate}
	\item \textbf{Hiệu quả vượt trội trên Drift mạnh (Severe/Sudden):}
	      Trên các dataset như \textit{gaussian\_shift}, \textit{stagger}, và \textit{gen\_random\_severe}, phương pháp đề xuất (IDW-MMD, ShapeDD\_WMMD\_PROPER) đạt F1 gần như tuyệt đối ($>0.99$).
	...
	\item \textbf{Thách thức với Drift nhẹ (Mild Drift) và Cơ chế IDW-MMD:}
	      Trên tập \textit{gen\_random\_mild}, hiệu suất của MMD\_IDW giảm mạnh (F1 = 0.126) so với MMD truyền thống (F1 = 0.581).
	...
	\item \textbf{Blip Drift:}
	      Trên tập \textit{rbfblips}, các phương pháp sử dụng adaptive bandwidth (như MMD-Agg) về lý thuyết có thể phát hiện tốt các blip. Tuy nhiên, trong thực nghiệm, MMD-Agg đã bị loại khỏi benchmark do thời gian tính toán quá cao (~500ms/window). Phương pháp IDW-MMD với median heuristic vẫn có khả năng phát hiện các blip mạnh.
	      \textit{Lý giải:} Các blip là những thay đổi ngắn hạn, cục bộ. Việc sử dụng trọng số nghịch mật độ giúp tăng độ nhạy với các thay đổi ở vùng biên phân phối.

	\item \textbf{Phân tích Báo động giả (False Positives):}
	      Hình~\ref{fig:stationary_fp} so sánh số lượng báo động giả trên tập dữ liệu tĩnh (\textit{stagger_none}). KS-Test tạo ra lượng báo động sai lớn nhất, khẳng định tính nhạy cảm quá mức với nhiễu ngẫu nhiên. Ngược lại, ShapeDD\_ADW\_MMD và D3 duy trì được độ ổn định (0 False Positives), nhờ vào cơ chế kiểm định chặt chẽ hơn (trọng số tối ưu và mô hình phân loại tương ứng).
\end{enumerate}
...
\input{../../experiments/benchmark/publication_figures/table_III_runtime_stats.tex}

\textbf{Các kết quả đáng chú ý:}
\begin{itemize}
	\item \textbf{Tăng tốc gấp hơn 7 lần:} ShapeDD\_WMMD\_PROPER đạt thông lượng $\sim 50,300$ mẫu/giây, nhanh gấp 7.2 lần so với ShapeDD gốc ($\sim 7,000$ mẫu/giây).
...
\begin{itemize}
	\item \textbf{Hiệu ứng làm sắc nét của IDW-MMD:} Kết quả cho thấy tỷ lệ phân loại Gradual Drift chỉ đạt 46.2\%. Nguyên nhân chính là do cơ chế trọng số nghịch mật độ của IDW-MMD hoạt động ``quá hiệu quả'', phát hiện sự thay đổi nhỏ nhất ngay khi nó xuất hiện, biến ``mound'' rộng của Gradual thành peak sắc nét giống Sudden.
	\item \textbf{Phân biệt Incremental và Gradual:} Mặc dù độ chính xác Subcategory của Incremental thấp (8.5\%), nhưng độ chính xác Category (nhận diện là PCD) đạt tới 80.0\%. Hệ thống thường nhầm Incremental thành Gradual do nhiễu trong tín hiệu IDW tạo ra các ``pseudo-peaks'' (đỉnh giả) có độ rộng lớn.
...
\begin{table}[H]
	...
	\end{tabular}
	\begin{flushleft}
		\small *Dựa trên trung bình trọng số của Sudden và Incremental/Gradual trong thực nghiệm. (Lưu ý: Kết quả này cao hơn mức trung bình 74.6\% của benchmark tổng hợp do tập trung vào các trường hợp drift điển hình).
	\end{flushleft}
\end{table}
...
\begin{table}[H]
	\centering
	\caption{So sánh chi tiết CDT\_MSW, SE-CDT và biến thể ADW-MMD theo loại drift}
...
\begin{table}[H]
	...
	\input{../../report/latex/tables/table_comparison_aggregate.tex}
	\begin{flushleft}
		\small \textit{Lưu ý: Độ chính xác CAT 74.6\% ở đây khác với 78.0\% trong Bảng~\ref{tab:se-cdt-comparison} do sự khác biệt về phạm vi đánh giá. Bảng tổng hợp này tính trung bình trên toàn bộ 10 datasets, trong khi Bảng~\ref{tab:se-cdt-comparison} tập trung vào các kịch bản so sánh trực tiếp với CDT\_MSW.}
	\end{flushleft}
\end{table}
...
\begin{itemize}
	\item \textbf{IDW-MMD CAT Accuracy:} 20.0\% (so với 78.0\% của standard MMD)
	\item \textbf{IDW-MMD SUB Accuracy:} 6.7\% (so với 44.2\% của standard MMD)
...
\begin{itemize}
	\item \textbf{IDW-MMD tốt cho detection} (phát hiện có/không có drift) với F1 = 0.562 (benchmark Chương 4.3)
...
\begin{itemize}
	\item \textbf{Sudden \& Blip (TCD):} SE-CDT đạt 100\% CAT accuracy, CDT\_MSW đạt 67\%
	\item \textbf{Recurrent:} SE-CDT đạt 100\% CAT accuracy với peaks đều đặn
	\item \textbf{Gradual \& Incremental (PCD):} Tất cả phương pháp đều gặp khó khăn (Circle CAT = 0\%)
\end{itemize}

\textbf{Kết luận chung:}
\begin{itemize}
	\item \textbf{SE-CDT đạt độ chính xác nhóm (CAT) cao nhất (78.0\%)} mà không cần labels.
	\item \textbf{CDT\_MSW nhạy hơn về phát hiện drift} (MDR = 0.267 vs 0.678) do có sự hỗ trợ của nhãn mô hình.
	\item \textbf{Đánh đổi (Trade-off):} SE-CDT ưu thế về phân loại, CDT\_MSW ưu thế về phát hiện.
	\item \textbf{IDW-MMD phù hợp cho phát hiện (detection), không phù hợp cho phân loại (classification).}
\end{itemize}
...
\begin{enumerate}
	\item \textbf{Về chất lượng phát hiện:} \textbf{MMD\_IDW} và \textbf{ShapeDD\_WMMD\_PROPER} là những phương pháp hiệu quả nhất tổng thể, đặc biệt xuất sắc với các drift mạnh và đột ngột. Tuy nhiên, cần lưu ý sự đánh đổi độ nhạy đối với các drift quá nhẹ (mild drift).
	\item \textbf{Về hiệu năng hệ thống:} Cải tiến IDW-MMD với phân phối tiệm cận mang lại lợi ích to lớn về mặt tính toán, tăng thông lượng xử lý lên \textbf{hơn 7 lần}, biến ShapeDD từ một thuật toán nghiên cứu thành một giải pháp khả thi cho môi trường sản xuất (production-ready).
	...
	\item \textbf{Về tính toàn vẹn:} D3 tuy nhanh nhưng hạn chế với Virtual Drift, trong khi KS tuy nhạy nhưng quá ồn (nhiều False Positive). ShapeDD\_WMMD\_PROPER cung cấp sự cân bằng tốt nhất: nhanh, chính xác và ít báo động giả.
\end{enumerate}

Kết quả này khẳng định hướng tiếp cận sử dụng MMD có trọng số nghịch mật độ (inverse density weighting) kết hợp với phân tích hình dạng và asymptotic p-value là một hướng đi đúng đắn để giải quyết bài toán Concept Drift trong dữ liệu lớn.
