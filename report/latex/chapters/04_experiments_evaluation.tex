...
\begin{itemize}
	...
	\item \textbf{SE\_CDT và ShapeDD\_WMMD\_PROPER (F1 = 0.481):} Hai phương pháp đề xuất đạt Precision cao (0.539) với \textbf{không có báo động giả} (FP = 0). Đây là điểm mạnh quan trọng cho các hệ thống yêu cầu độ tin cậy cao, nơi chi phí của False Alarm cao. Độ trễ phát hiện thấp nhất (18 mẫu) cho thấy khả năng phản ứng nhanh.
...
\begin{enumerate}
	...
	\item \textbf{Phân tích Báo động giả (False Positives):}
	      Hình~\ref{fig:stationary_fp} so sánh số lượng báo động giả. KS-Test tạo ra lượng báo động sai lớn nhất (36.6 FP/run), khẳng định tính nhạy cảm quá mức với nhiễu ngẫu nhiên. Ngược lại, SE\_CDT và ShapeDD\_WMMD\_PROPER duy trì được \textbf{0 False Positives} trên toàn bộ benchmark, nhờ vào cơ chế shape detection chặt chẽ.
\end{enumerate}
...
\begin{itemize}
	...
	\item \textbf{So sánh với baseline:} MMD chuẩn đạt 28,818 mẫu/giây (4.5× speedup), KS đạt 46,729 mẫu/giây (7.3× speedup). SE\_CDT vượt trội cả hai về tốc độ.
...
\begin{itemize}
	\item \textbf{Sudden \& Blip (TCD):} SE-CDT (Std) đạt EDR = 94.4\%, vượt trội so với CDT\_MSW (14.4\%)
...
\begin{enumerate}
	...
	\item \textbf{Về hiệu năng hệ thống:} Cải tiến IDW-MMD với phân phối tiệm cận mang lại lợi ích to lớn về mặt tính toán, tăng thông lượng xử lý lên \textbf{17--20 lần} (từ $\sim$8,000 lên $\sim$131,000 mẫu/giây), biến ShapeDD từ một thuật toán nghiên cứu thành một giải pháp khả thi cho môi trường sản xuất (production-ready).
	\item \textbf{Về phân loại drift:} SE-CDT (Std) đạt \textbf{CAT = 85.8\%} và \textbf{SUB = 46.6\%} mà không cần labels, vượt trội so với CDT\_MSW supervised (CAT = 38.7\%). SE-CDT (ADW) cung cấp trade-off với FP thấp hơn (171 vs 1394) nhưng EDR giảm (50.6\% vs 94.4\%).
