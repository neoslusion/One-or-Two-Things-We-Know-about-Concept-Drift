...
\begin{itemize}
	\item \textbf{SE\_CDT và ShapeDD\_WMMD\_PROPER dẫn đầu về F1 (0.481):} Cả hai phương pháp đề xuất đạt F1-score cao nhất với Precision tốt (0.539) và không có báo động giả (False Positives = 0). Độ trễ phát hiện thấp (Mean Delay = 18 mẫu) cho thấy khả năng phản ứng nhanh với drift.
	\item \textbf{MMD baseline xếp thứ 3 (F1 = 0.455):} Phương pháp MMD chuẩn đạt Recall cao hơn (0.722) nhưng Precision thấp hơn (0.348), dẫn đến 14.3 báo động giả trung bình mỗi lần chạy.
...
Mặc dù p-value tổng thể của kiểm định Friedman có thể cho thấy sự khác biệt giữa các nhóm chưa đạt mức ý nghĩa thống kê cao (do phương sai lớn giữa các dataset), thứ hạng trung bình (Rank) của SE\_CDT và ShapeDD\_WMMD\_PROPER ổn định hơn hẳn so với các phương pháp khác, đặc biệt khi xét đến số lượng báo động giả thấp.
...
\begin{table}[H]
	...
	\input{../../results/tables/table_comparison_aggregate.tex}
	\begin{flushleft}
