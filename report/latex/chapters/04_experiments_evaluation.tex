...
\input{../../experiments/drift_detection_benchmark/publication_figures/table_II_f1_by_dataset.tex}
	...
	\caption{Minh họa tín hiệu phát hiện trên dữ liệu Sudden Drift (trong kịch bản Mixed A). ShapeDD tạo ra hình dạng "tam giác cân" đặc trưng (Subplot 03) tại điểm drift, đúng như dự báo lý thuyết.}
...
\begin{figure}[h]
	...
	\caption{Minh họa tín hiệu trên dữ liệu Gradual Drift. Tín hiệu (màu đỏ - Subplot 03) không tạo thành đỉnh nhọn rõ ràng mà bị san phẳng, khiến việc xác định ngưỡng trở nên khó khăn và dễ bị nhầm lẫn với nhiễu.}
...
\begin{table}[H]
	...
	\begin{tabular}{|l|c|c|}
		...
		Category accuracy & 46.7\%            & \textbf{78.0\%}   \\
...
\begin{itemize}
	\item \textbf{SHAPED\_CDT đạt Category Accuracy cao nhất (78.0\%)} mà không cần labels
...
\textbf{Kết luận quan trọng:} Ngay cả khi CDT\_MSW được chạy với \textbf{labels đúng} (decision boundary thay đổi khi concept thay đổi), nó chỉ đạt \textbf{32\% recall}. Trong khi SE-CDT không cần labels nhưng đạt \textbf{96\% recall}. Điều này chứng minh:
\begin{itemize}
	\item \textbf{Unsupervised approach outperforms supervised baseline 3$\times$} trong detection
	\item CDT\_MSW yêu cầu model SVC học được concept, với window\_size nhỏ (50 samples) không đủ data
	\item SE-CDT tận dụng trực tiếp sự thay đổi $P(X)$ qua MMD signal, không phụ thuộc vào model accuracy
\end{itemize}
