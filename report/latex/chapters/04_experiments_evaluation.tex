...
\begin{enumerate}
	...
	\item \textbf{Phân tích Báo động giả (False Positives):}
	      Hình~\ref{fig:stationary_fp} so sánh số lượng báo động giả \textit{trong detection benchmark} (có drift hay không). KS-Test tạo ra lượng báo động sai lớn nhất (36.6 FP/run). SE-CDT và ShapeDD-IDW duy trì \textbf{0 False Positives} trên tất cả các dataset có thay đổi $P(X)$, nhờ vào cơ chế shape detection chặt chẽ. \textit{Lưu ý:} Khái niệm FP ở đây khác với False Positives trong bài toán classification (Mục~\ref{sec:classification-eval}), nơi FP phản ánh lỗi phân loại loại drift.
\end{enumerate}
...
Hình runtime ở trên minh họa sự cải thiện rõ rệt về thời gian xử lý giữa các phương pháp, trong khi chi tiết thông lượng cụ thể được liệt kê trong Bảng~\ref{tab:runtime_stats}.
...
\textbf{Cấu hình thực nghiệm như sau:}
\begin{itemize}
...
\textit{Lưu ý: 85.8\% là kết quả đo trên tập kiểm thử sơ bộ (quick validation) để lựa chọn biến thể MMD. Kết quả cuối cùng \textbf{78.2\%} trong Bảng~\ref{tab:se-cdt-results} được đo trên benchmark đầy đủ (170 kịch bản, oracle mode, $\tau=0.015$) --- chế độ khắt khe hơn với nhiều cấu hình drift đa dạng hơn. Sự chênh lệch phản ánh độ khó gia tăng khi đánh giá trên toàn bộ benchmark thay vì tập sơ bộ.}
...
\begin{figure}[H]
	...
	\caption[Confusion Matrix - 5 Classes]{
		\textbf{Confusion Matrix cho 5 loại drift} (oracle mode: sử dụng vị trí drift ground-truth): Sudden có recall cao nhất (xấp xỉ 81.8\%), trong khi Incremental và Gradual thường bị nhầm lẫn do tín hiệu MMD tương tự nhau.
	}
...
\begin{itemize}
	\item \textbf{Kết quả độ chính xác:} SE-CDT (Standard MMD) đạt độ chính xác nhóm (CAT) = \textbf{78.2\%} và độ chính xác subcategory (SUB) = \textbf{49.8\%}. Lưu ý: kết quả phân loại được tính tại các vị trí drift đã biết (ground-truth positions) để đánh giá khả năng classify ở điều kiện tốt nhất.
	\item \textbf{Tỷ lệ phát hiện sự kiện (Event Detection Rate - tương đương với Recall):} Đạt \textbf{94.4\%}, nghĩa là phát hiện phần lớn các drift. Đổi lại, số lượng báo động giả (FP) cao hơn so với IDW-MMD (\textbf{1394} vs 171), nhưng trong ngữ cảnh classification (sau khi detection module đã lọc), ưu tiên là không bỏ sót thông tin để phân loại đúng.
...
Để đánh giá toàn diện khả năng của SE-CDT trong môi trường không giám sát, phần này phân tích hiệu suất trên từng loại drift cụ thể. Bảng~\ref{tab:se-cdt-performance-summary} tóm tắt các chỉ số hiệu năng chính của phương pháp.
...
Kết quả cho thấy SE-CDT đạt độ chính xác phân loại nhóm (Category Accuracy) là \textbf{78.2\%} và độ chính xác chi tiết (Subtype Accuracy) là \textbf{49.8\%}. Đặc biệt, tỷ lệ phát hiện sự kiện (Event Detection Rate - tương đương với Recall) đạt mức rất cao \textbf{94.4\%}, cho thấy phương pháp rất nhạy trong việc bắt được các thời điểm thay đổi.

\textit{So sánh với State-of-the-Art:} Trong nghiên cứu gốc đề xuất CDT-MSW~\cite{guo2022cdtmsw} (phương pháp supervised yêu cầu nhãn lớp), các tác giả báo cáo độ chính xác phân loại nhóm \textbf{87.0\%} và độ chính xác phân loại chi tiết \textbf{86.7\%} (điều kiện: tính trên các drift được phát hiện và phân nhóm đúng). Tuy nhiên, tỷ lệ phát hiện drift (EDR) của CDT-MSW chỉ đạt trung bình \textbf{17.8\%} vì phương pháp ưu tiên độ chính xác thay vì độ bao phủ. SE-CDT, hoạt động hoàn toàn \textbf{không giám sát} và không yêu cầu nhãn dữ liệu, đạt độ chính xác phân loại nhóm \textbf{78.2\%} và EDR vượt trội \textbf{94.4\%}, cho thấy phương pháp dựa trên đặc trưng hình học MMD (WR, PPR, DPAR) là lựa chọn hiệu quả cho môi trường thực tế không có nhãn. Chi tiết so sánh theo từng loại drift được trình bày trong Bảng~\ref{tab:cdt-comparison-by-type}.

\begin{table}[H]
	\centering
	\caption{So sánh độ chính xác phân loại chi tiết theo từng loại drift: CDT-MSW (Guo et al. 2022) vs SE-CDT. Lưu ý: ACC\_subcat của CDT-MSW là điều kiện (tính trên các drift phát hiện đúng vị trí và phân nhóm đúng).}
...
Ngoài ra, hệ thống tích hợp cơ chế \textbf{reset bộ đệm theo mẫu ADWIN} (Adaptive Windowing): sau mỗi lần thích ứng, bộ đệm được xóa và chỉ giữ lại 200 mẫu gần nhất làm tham chiếu mới. Cơ chế này loại bỏ hoàn toàn hiện tượng phát hiện lại cùng một drift event, đạt \textbf{0 False Positives} cho tất cả các chiến lược thích ứng.
...
\begin{table}[H]
	\centering
	\caption{Kết quả Prequential Accuracy theo loại drift ($N=5$ runs, $T=5000$ mẫu)}
