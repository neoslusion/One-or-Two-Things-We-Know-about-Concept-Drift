...
% =============================================================================
\input{tables/table_I_comprehensive_performance.tex}
...
% =============================================================================
\input{tables/table_II_f1_by_dataset.tex}

\begin{figure}[h]
	\centering
	\includegraphics[width=0.9\textwidth]{../../experiments/publication_figures/vis_mixed_a_SE.png}
...
\begin{enumerate}
	\item \textbf{Hiệu quả vượt trội trên Drift mạnh (Severe/Sudden):}
	      Trên các dataset như \textit{gaussian_shift}, \textit{stagger}, và \textit{gen_random_severe}, các phương pháp đề xuất (ADW-MMD, ShapeDD\_MMDAgg) đạt F1 gần như tuyệt đối ($>0.99$).
	...
	\item \textbf{Thách thức với Drift nhẹ (Mild Drift) và Cơ chế ADW-MMD:}
	      Trên tập \textit{gen_random_mild}, hiệu suất của MMD\_ADW giảm mạnh (F1 = 0.126) so với MMD truyền thống (F1 = 0.581).
...
\input{tables/table_III_runtime_stats.tex}
...
\begin{table}[H]
	...
	\input{tables/table_comparison_aggregate.tex}
	\begin{flushleft}
...
Trong quá trình nghiên cứu, luận văn cũng thử nghiệm biến thể sử dụng \textbf{Adaptive Density-Weighted MMD (ADW-MMD)} thay cho standard MMD trong SE-CDT. Mục đích ban đầu là tận dụng các ưu điểm của ADW-MMD về tốc độ và độ chính xác phát hiện để cải thiện khả năng phân loại.
...
\begin{enumerate}
	...
	\item \textbf{Tương thích với ADW-MMD signal:} Cơ chế trọng số nghịch mật độ của ADW-MMD tạo ra tín hiệu có biên độ nhỏ hơn standard MMD. Ngưỡng 0.15 phù hợp với phân phối thực tế của ADW-MMD output.
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\textwidth]{figures/kafka_demo/prequential_accuracy_sudden.png}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\textwidth]{figures/kafka_demo/prequential_accuracy_gradual.png}
...
\begin{enumerate}
	\item \textbf{Về chất lượng phát hiện:} \textbf{MMD\_ADW} và \textbf{ShapeDD\_ADW\_MMD} là những phương pháp hiệu quả nhất tổng thể, đặc biệt xuất sắc với các drift mạnh và đột ngột. Tuy nhiên, cần lưu ý sự đánh đổi độ nhạy đối với các drift quá nhẹ (mild drift).
	...
	\item \textbf{Về tính toàn vẹn:} D3 tuy nhanh nhưng hạn chế với Virtual Drift, trong khi KS tuy nhạy nhưng quá ồn (nhiều False Positive). ShapeDD\_ADW\_MMD cung cấp sự cân bằng tốt nhất: nhanh, chính xác và ít báo động giả.
\end{enumerate}
