...
\begin{table}[H]
\centering
\caption{So sánh chi tiết CDT\_MSW, SHAPED\_CDT và biến thể OW-MMD theo loại drift}
...
\textbf{SHAPED\_CDT} & \textbf{80.0\%} & \textbf{20.0\%} & 0.915 & 0.678 & Không \\
SHAPED\_OW & 20.0\% & 6.7\% & 0.400 & 1.000 & Không \\
\hline
\end{tabular}
\end{table}

\subsection{Thử nghiệm biến thể OW-MMD}

Trong quá trình nghiên cứu, luận văn cũng thử nghiệm biến thể sử dụng \textbf{Optimally-Weighted MMD (OW-MMD)} thay cho standard MMD trong SHAPED\_CDT. Mục đích ban đầu là tận dụng các ưu điểm của OW-MMD về tốc độ và độ chính xác phát hiện để cải thiện khả năng phân loại.

\textbf{Kết quả thử nghiệm:}
\begin{itemize}
    \item \textbf{OW-MMD CAT Accuracy:} 20.0\% (so với 80.0\% của standard MMD)
    \item \textbf{OW-MMD SUB Accuracy:} 6.7\% (so với 20.0\% của standard MMD)
    \item \textbf{MDR = 1.0:} Bỏ lỡ tất cả các điểm drift trên dataset PCD (Gradual, Incremental)
\end{itemize}

\textbf{Phân tích nguyên nhân:}
OW-MMD sử dụng cơ chế \textit{variance reduction weighting} ($w_{ij} \propto 1/\sqrt{d_i}$), gán trọng số thấp cho các điểm trong vùng mật độ cao. Điều này có hai hệ quả:
\begin{enumerate}
    \item \textbf{Hiệu quả cho TCD (Sudden, Blip):} Các thay đổi đột ngột tạo ra sự khác biệt rõ ràng giữa reference và test windows, OW-MMD phát hiện tốt.
    \item \textbf{Không hiệu quả cho PCD (Gradual, Incremental):} Các thay đổi diễn ra từ từ, từng bước nhỏ. OW-MMD coi những thay đổi nhỏ này là ``noise'' và loại bỏ, dẫn đến signal = 0.
\end{enumerate}

\textbf{Kết luận từ thử nghiệm:}
\begin{itemize}
    \item \textbf{OW-MMD tốt cho detection} (phát hiện có/không có drift) với F1 = 0.562 (benchmark Chương 4.3)
...
\begin{itemize}
    ...
    \item \textbf{OW-MMD phù hợp cho detection, không phù hợp cho classification}
\end{itemize}
