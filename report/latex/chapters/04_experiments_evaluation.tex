...
\begin{itemize}
	\item \textbf{MMD\_IDW (IDW-MMD)}: Inverse Density-Weighted MMD, sử dụng trọng số nghịch mật độ.
	\item \textbf{ShapeDD\_WMMD\_PROPER (ShapeDD-IDW)}: ShapeDD với IDW-MMD và asymptotic p-value.
	\item \textbf{SE\_CDT (SE-CDT)}: Hệ thống thống nhất phát hiện và phân loại drift.
\end{itemize}
...
Để có cái nhìn khách quan, SE-CDT được đối chiếu với CDT-MSW~\cite{guo2022cdtmsw} --- một phương pháp supervised tiêu biểu. Nghiên cứu thực hiện so sánh kép: (1) so với kết quả tự thực nghiệm (local baseline) và (2) so với các giá trị được công bố trong bài báo gốc (paper values).

Bảng~\ref{tab:cdt-comparison-main} so sánh SE-CDT với kết quả của CDT-MSW.

\begin{table}[H]
	\centering
	\caption{So sánh SE-CDT với CDT-MSW (Tổng hợp)}
...
\textbf{Nhận xét:}
\begin{itemize}
    \item \textbf{Vượt trội so với local baseline:} Trong cùng điều kiện thử nghiệm trên bộ benchmark đa dạng, SE-CDT (81.2\% CAT) vượt xa phiên bản thực nghiệm của CDT-MSW (53.2\% CAT). Điều này cho thấy tính ổn định của phương pháp unsupervised khi đối mặt với nhiều kịch bản drift phức tạp.
    \item \textbf{So với paper values:} Mặc dù độ chính xác CAT của SE-CDT (81.2\%) thấp hơn nhẹ so với giá trị được trình bày trong bài báo gốc của CDT-MSW (87.5\%), SE-CDT lại vượt trội về độ chính xác phân loại chi tiết (SUB Accuracy 50\% so với 42\%) và khả năng phát hiện sự kiện (EDR 94.4\% so với 80\%).
...
Để hiểu sâu hơn về khả năng phân loại của SE-CDT, Bảng~\ref{tab:cdt-per-type} trình bày hiệu suất trên từng loại drift cụ thể so với paper values của CDT-MSW.
...
Để khắc phục hạn chế của các bộ tạo dữ liệu Sudden Drift truyền thống (thường luân phiên giữa hai concept A $\to$ B $\to$ A, dẫn đến việc mô hình tĩnh có độ chính xác ảo ~75\%), luận văn bổ sung kịch bản **Stepping Drift**. Đây là dạng trôi dạt tích lũy (Cumulative Sudden Drift), nơi phân phối dữ liệu liên tục dịch chuyển ra xa trạng thái ban đầu ($P_0 \to P_1 \to P_2 \dots$), buộc mô hình phải học lại liên tục để duy trì hiệu suất.
