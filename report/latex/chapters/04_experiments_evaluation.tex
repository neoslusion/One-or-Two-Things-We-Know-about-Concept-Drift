...
Kết quả kiểm định Friedman cho thấy MMD cơ bản có thứ hạng trung bình tốt nhất (3.64), tiếp theo là IDW-MMD (3.91). SE-CDT xếp hạng 4 (4.50). Với ngưỡng Critical Difference CD = 3.17 ($\alpha = 0.05$), hai phương pháp được coi là \textit{không khác biệt thống kê} khi khoảng cách thứ hạng nhỏ hơn CD. Khoảng cách giữa SE-CDT và phương pháp dẫn đầu (MMD) là $4.50 - 3.64 = 0.86 < 3.17$, do đó không tồn tại sự khác biệt có ý nghĩa thống kê về F1 giữa các phương pháp nhóm dẫn đầu. Điều quan trọng hơn là \textbf{SE-CDT/ ShapeDD-IDW đạt FP = 0} --- yếu tố không được phản ánh trong F1 nhưng có giá trị thực tiễn lớn trong môi trường production.
...
\begin{itemize}
	\item \textbf{Tăng tốc gấp 16 lần:} SE\_CDT đạt thông lượng $\sim 131,500$ mẫu/giây (7.6ms/window), nhanh gấp \textbf{16.5 lần} so với ShapeDD gốc ($\sim 7,987$ mẫu/giây, 125.2ms/window).
...
\textit{Lưu ý: 85.8\% là kết quả đo trên tập kiểm thử sơ bộ (quick validation) để lựa chọn biến thể MMD. Kết quả cuối cùng \textbf{77.4\%} trong Bảng~\ref{tab:se-cdt-results} được đo trên benchmark đầy đủ (170 kịch bản, oracle mode, $\tau=0.015$) --- chế độ khắt khe hơn với nhiều cấu hình drift đa dạng hơn. Sự chênh lệch phản ánh độ khó gia tăng khi đánh giá trên toàn bộ benchmark thay vì tập sơ bộ.}
...
\begin{figure}[H]
	...
	\caption[Confusion Matrix - 5 Classes]{
		\textbf{Confusion Matrix cho 5 loại drift} (oracle mode: sử dụng vị trí drift ground-truth): Sudden có recall cao nhất (82.4\%, 140/170 events), trong khi Incremental và Gradual thường bị nhầm lẫn do tín hiệu MMD tương tự nhau.
	}
...
\begin{figure}[H]
	...
	\caption[Confusion Matrix - Category Level]{
		\textbf{Confusion Matrix ở mức Category (TCD vs PCD)} (oracle mode): Cho thấy độ chính xác 77.4\%, với TCD recall = 90.4\% và PCD recall = 72.7\%.
	}
	\label{fig:confusion-category}
\end{figure}

\begin{itemize}
	\item \textbf{Kết quả độ chính xác:} SE-CDT (Standard MMD) đạt độ chính xác nhóm (CAT) = \textbf{77.4\%} và độ chính xác subcategory (SUB) = \textbf{50.0\%}. Lưu ý: kết quả phân loại được tính tại các vị trí drift đã biết (ground-truth positions) để đánh giá khả năng classify ở điều kiện tốt nhất.
...
\begin{itemize}
	\item \textbf{Sudden Drift (82.4\%):} Đây là điểm mạnh nhất của phương pháp. Tín hiệu MMD tạo ra đỉnh nhọn (peak) rõ ràng, giúp dễ dàng phân biệt với các loại khác.
...
\begin{table}[H]
	...
	\begin{tabular}{|l|c|c|c|c|c|}
		...
		Sudden (Alternating)  & \textbf{78.40\%} $\pm$1.1 & 64.76\% $\pm$3.3 & 70.97\% $\pm$1.8 & 55.16\% $\pm$7.8 & 0 / 0 / 0 / 15 \\
...
\begin{itemize}
	...
	\item \textbf{Sudden Drift (+23.24\%):} Với bộ sinh dữ liệu Joint Drift (kết hợp thay đổi $P(X)$ và $P(Y|X)$), No Adaptation giảm về 55.16\%. Type-Specific đạt 78.40\%, vượt trội rõ rệt so với tất cả các baseline.
...
Trên kịch bản Sudden Drift sử dụng Joint Drift Generator (kết hợp thay đổi $P(X)$ và $P(Y|X)$), No Adaptation giảm mạnh về 55.16\% (thay vì $\sim$75\% trước đây nhờ concept lặp lại) vì phân phối dữ liệu thay đổi toàn diện. Type-Specific đạt 78.40\%, vượt trội so với tất cả baseline, chứng minh hiệu quả của chiến lược thích ứng có chọn lọc.
...
\begin{enumerate}
	...
	\item \textbf{Classification:} Module SE-CDT (sử dụng Standard MMD) cho thấy khả năng phân loại nhóm drift đạt CAT = 77.4\% và phân loại chi tiết SUB = 50.0\% mà không cần dán nhãn dữ liệu, với tỷ lệ phát hiện sự kiện (EDR - tương đương với Recall) đạt 94.4\%.
