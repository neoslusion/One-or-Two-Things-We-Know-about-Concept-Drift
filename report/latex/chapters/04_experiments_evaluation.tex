...
\begin{figure}[H]
	...
	\caption[Confusion Matrix - Category Level]{
		\textbf{Confusion Matrix ở mức Category (TCD vs PCD)} (oracle mode): Cho thấy độ chính xác 81.2\%, với TCD recall = 90.4\% và PCD recall = 72.7\%.
	}
...
\textbf{Phân tích:}
\begin{itemize}
	\item \textbf{Kết quả độ chính xác:} SE-CDT (Standard MMD) đạt độ chính xác nhóm (CAT) = \textbf{81.2\%} và độ chính xác subcategory (SUB) = \textbf{50.0\%}. Lưu ý: kết quả phân loại được tính tại các vị trí drift đã biết (ground-truth positions) để đánh giá khả năng classify ở điều kiện tốt nhất.
...
Kết quả cho thấy SE-CDT đạt độ chính xác phân loại nhóm (Category Accuracy) là \textbf{81.2\%} và độ chính xác chi tiết (Subtype Accuracy) là \textbf{50.0\%}. Đặc biệt, tỷ lệ phát hiện sự kiện (Event Detection Rate - tương đương với Recall) đạt mức rất cao \textbf{94.4\%}, cho thấy phương pháp rất nhạy trong việc bắt được các thời điểm thay đổi.

\textit{So sánh với State-of-the-Art:} Trong nghiên cứu gốc đề xuất CDT-MSW~\cite{guo2022cdtmsw} (phương pháp semi-supervised sử dụng labels để xác định drift), các tác giả báo cáo độ chính xác phân loại nhóm khoảng 87.5\% và độ chính xác chi tiết khoảng 42.0\%. SE-CDT đạt kết quả tương đương về phân loại nhóm (81.2\%) và vượt trội về phân loại chi tiết (50.0\%), mặc dù hoạt động hoàn toàn \textbf{không giám sát}. Điều này khẳng định hiệu quả của việc sử dụng các đặc trưng hình học từ tín hiệu MMD (như WR, PPR, DPAR) để thay thế cho labels trong bài toán định danh drift.
...
\begin{enumerate}
	...
	\item \textbf{Classification:} Module SE-CDT (sử dụng Standard MMD) cho thấy khả năng phân loại nhóm drift đạt CAT = 81.2\% và phân loại chi tiết SUB = 50.0\% mà không cần dán nhãn dữ liệu, với tỷ lệ phát hiện sự kiện (EDR - tương đương với Recall) đạt 94.4\%.
