...
Chẳng hạn, kiểm định PH~\cite{page1954continuous,hinkley1971cumulative} được áp dụng trong bối cảnh concept drift để giám sát trung bình của một chuỗi
...
DDM (Drift Detection Method) theo dõi tỷ lệ lỗi $p_i$ và độ lệch chuẩn $s_i$ của mô hình trên dòng dữ liệu.
...
EDDM (Early Drift Detection Method) cải thiện khả năng phát hiện gradual drift bằng cách thay đổi metric theo dõi. Thay vì theo dõi trực tiếp tỷ lệ lỗi, EDDM tập trung
...
HDDM (Hoeffding's Drift Detection Method)~\cite{friasblanco2015hddm} là một hướng cải tiến quan trọng, áp dụng các công cụ thống kê chặt chẽ hơn để giảm
...
FHDDM (Fast HDDM)~\cite{pesaranghader2016fhddm} được giới thiệu như phiên bản tăng tốc của HDDM, cải thiện cả tốc độ và độ nhạy trong việc phát hiện drift.
...
MDDM (McDiarmid's Drift Detection Method)~\cite{pesaranghader2018mcdiarmid} là phương pháp áp dụng bất đẳng thức McDiarmid -- một công cụ thống kê dạng concentration inequality -- để phát hiện những thay đổi đáng kể trong phân phối dữ liệu. Điểm đặc biệt của MDDM là việc áp dụng các lược đồ trọng số (weighting schemes)
...
RDDM (Reactive DDM)~\cite{barros2017rddm} tập trung vào việc tối ưu thời gian phản ứng bằng cách rút ngắn giai đoạn cảnh báo (warning phase) của DDM gốc.
...
Những năm gần đây còn xuất hiện các biến thể hướng đến các tình huống đặc thù (xem Bảng~\ref{tab:ddm_comparison} để so sánh tổng hợp):
\begin{itemize}
...
Nhóm phương pháp dựa trên hiệu năng mô hình bắt đầu từ DDM~\cite{gama2004learning} và đã được mở rộng thành nhiều biến thể cân bằng hơn giữa tốc độ, độ chính xác và độ tin cậy.
...
mô hình học máy hoặc tổ hợp nhiều mô hình. Những phương pháp này thường có tính ``chủ động'' hơn: thay vì chờ hiệu năng giảm, chúng tìm
...
phương pháp \textbf{one-class} như OCDD (One-Class Drift Detector)~\cite{gozuacik2021ocdd} dùng một bộ phân lớp one-class huấn luyện trên cửa sổ dữ liệu cũ; khi tỉ lệ mẫu mới bị bộ phân lớp này xem là ngoại lai tăng vượt ngưỡng, đó là dấu hiệu drift.
...
\textbf{Hướng (III): Intrinsic detection.} Khai thác trực tiếp năng lực biểu diễn của mô hình deep để nhận biết drift. Ví dụ, một autoencoder có thể học nén dữ liệu đầu vào; nếu phân phối dữ liệu thay đổi, lỗi tái tạo (reconstruction error) của autoencoder sẽ tăng cao. Jaworski và cộng sự~\cite{jaworski2020aedetect} giám sát phân phối lỗi tái tạo của autoencoder, khi phân phối này dịch chuyển nhiều thì phát tín hiệu drift. Trong lĩnh vực chuỗi thời gian, một hướng tương tự là học mô hình chuỗi bằng LSTM rồi dùng sai số dự báo làm tín hiệu phát hiện thay đổi~\cite{yuan2022advances}; kỹ thuật đặt ngưỡng động phi tham số (\textit{nonparametric dynamic thresholding}) cho loại tín hiệu sai số này được Hundman và cộng sự~\cite{hundman2018lstmndt} giới thiệu trong bài toán phát hiện bất thường trên dữ liệu viễn trắc, và về sau được vận dụng lại cho bài toán phát hiện drift. Các phương pháp này tận dụng được thông tin trong dữ liệu nhờ đặc trưng học được của network, nhưng khó tách bạch giữa ``drift'' và ``nhiễu'' bên trong mô hình.
...
\begin{table}[H]
	...
	\begin{tabular}{@{}lp{2.8cm}p{2.5cm}p{3cm}p{3cm}@{}}
		...
		\addlinespace
		\textbf{D3}         & Virtual Classifier          & Unsupervised Separation  & Tốc độ nhanh, không cần nhãn~\cite{gozuacik2019d3}            & Hiệu quả phụ thuộc mạnh vào lớp mô hình phân biệt được chọn~\cite{hinder2024survey_partA} \\
		\addlinespace
		...
		\addlinespace
		\textbf{DAWIDD}     & Block-Based                 & Independence Test (HSIC --- Hilbert-Schmidt Independence Criterion) & Ít giả định nhất về dữ liệu và dạng drift, nên bao phủ được nhiều loại drift~\cite{hinder2020dawidd, hinder2024survey_partA} & Cần nhiều dữ liệu hơn để đạt cùng độ tin cậy; chi phí tính toán cao (Bảng~\ref{tab:runtime_stats})~\cite{hinder2024survey_partA} \\
		\bottomrule
	\end{tabular}
\end{table}
...
\begin{table}[!htbp]
	\centering
	\caption{Tổng hợp hiệu năng phát hiện drift từ các nghiên cứu gốc}
...
Như bảng~\ref{tab:drift-history} tóm tắt, lĩnh vực phát hiện concept drift đã đi từ các ý tưởng cửa sổ trượt và giám sát sai số mô hình sang các kiểm định thống kê, ensemble, meta-learning và gần đây là học sâu~\cite{hovakimyan2024evolving}. Xu hướng chung là giảm phụ thuộc vào giả định cứng, xử lý được nhiều dạng drift hơn, và chuẩn bị cho các hệ thống vừa phát hiện vừa thích ứng với dữ liệu biến đổi.

\section{Phương pháp phân loại drift CDT-MSW}
\label{sec:cdt-msw-theory}

CDT-MSW~\cite{guo2022cdtmsw} là phương pháp \emph{có giám sát} phân loại loại drift dựa trên hành vi của độ chính xác mô hình trên nhiều cửa sổ trượt (Hình~\ref{fig:cdt_msw_overview}). Đây là nguồn cảm hứng trực tiếp cho SE-CDT (Chương~\ref{chap:proposed-model}); luận văn kế thừa hai ý chính sau và thay thế phần lõi tín hiệu, chi tiết kỹ thuật của ba giai đoạn xem trong bài báo gốc.

\textbf{Phân nhóm TCD vs PCD theo độ dài trôi dạt:} CDT-MSW dùng phương sai độ chính xác trên cửa sổ phụ $W_R$ để ước lượng độ dài $m$ của sự trôi dạt. Thủ tục ước lượng độ dài này được bài báo gốc gọi là \textbf{Growth process} (Algorithm 2 trong~\cite{guo2022cdtmsw}): sau khi phát hiện được vị trí drift, cửa sổ phụ được mở rộng dần cho tới khi phương sai độ chính xác trở lại mức ổn định, và số bước mở rộng chính là $m$. Luận văn giữ nguyên thuật ngữ này khi nhắc lại cơ chế của CDT-MSW ở Chương~\ref{chap:proposed-model} và Chương~\ref{chap:conclusion}. Khi độ dài $m=1$ (phân phối ổn định ngay lập tức), drift thuộc nhóm \textbf{Transient Concept Drift (TCD)} bao gồm Sudden, Blip và Recurrent. Khi $m>1$ (chuyển tiếp dần), drift thuộc nhóm \textbf{Progressive Concept Drift (PCD)} bao gồm Incremental và Gradual. Hai nhóm này được phân biệt tiếp ở giai đoạn theo dõi bằng hình dáng của đường cong Tracking Flow Ratio (TFR) và Micro TFR (MTFR).

\textbf{Hai mức đánh giá phân loại CAT và SUB:} Phân loại được đánh giá ở hai mức: nhóm chính TCD/PCD (gọi tắt là CAT) và tiểu loại Sudden/Blip/Recurrent/Incremental/Gradual (gọi tắt là SUB). Hai thuật ngữ CAT/SUB tái sử dụng trong các thí nghiệm phân loại của luận văn (Mục~\ref{sec:classification-eval}).
...
Tuy nhiên, việc dùng độ chính xác mô hình làm tín hiệu khiến CDT-MSW phụ thuộc vào nhãn $y$, không khả dụng cho môi trường không giám sát của luận văn. SE-CDT thay tín hiệu tỉ lệ độ chính xác bằng tín hiệu MMD đo trên cửa sổ trượt nhưng giữ lại cách phân nhóm CAT/SUB ở trên (Chương~\ref{chap:proposed-model}).
...
Tổng hợp từ các tài liệu trên và các khảo sát về học dưới concept drift~\cite{gama2014survey, lu2018learning, celik2020adaptation}, có thể chia các chiến lược cập nhật mô hình thành bốn nhóm chính:

\begin{enumerate}[label=(\alph*)]
	\item Cập nhật cục bộ (Local Reset / Fine-tuning) phù hợp với \emph{sudden drift}, khi phân phối dữ liệu thay đổi mạnh trong thời gian ngắn. Mô hình được tái huấn luyện một phần hoặc toàn bộ trên cửa sổ dữ liệu mới. Phương pháp này phù hợp với các bộ phát hiện trôi dạt dựa trên cửa sổ như ADWIN hoặc CDT-MSW, khi $L_d < \lambda$ (độ dài trôi dạt ngắn).
	...
	\item Cập nhật dần (Gradual / Incremental Update) được dùng khi mô hình gặp \emph{gradual} hoặc \emph{incremental drift}. Vì sự thay đổi diễn ra từ từ, mô hình thường cần cập nhật mềm với hệ số học $\eta_t$ nhỏ hơn. Một cách mô tả tổng quát là cập nhật tham số theo gradient trên dữ liệu mới với hệ số học hoặc trọng số quên phụ thuộc vào tốc độ drift:
	      \[
	...
	\item Cập nhật theo ngữ cảnh (Recurrent / Memory-based Adaptation) xử lý \emph{recurrent drift}, khi khái niệm cũ tái xuất hiện theo chu kỳ. Mô hình nên duy trì một \emph{bộ nhớ khái niệm} (concept repository), lưu các snapshot $\{M_i\}$ tương ứng với từng khái niệm và lựa chọn mô hình phù hợp nhất khi phân phối dữ liệu hiện tại $P_t(x)$ tương tự với một snapshot trước đó:
	      \[
	...
	\item Cập nhật cấu trúc (Structural or Hybrid Adaptation) thay đổi cả tham số lẫn \emph{cấu trúc mạng học sâu} khi có drift.
