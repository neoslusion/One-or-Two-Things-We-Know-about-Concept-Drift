...
\newcommand{\submissiondatevn}{tháng 6 năm 2026}
...
% \input{ext_pages/cover}          % "Trang bia va trang 1" -- the form repeats it
\input{ext_pages/committee}
\input{ext_pages/task_sheet_VN}  % BIEU MAU 5 -- the only task sheet the form asks for.
...
\input{ext_pages/abstract_VN}    % BIEU MAU 4: "Tom tat ... (tieng Viet va tieng Anh)"
...
% Prefix each entry with "Bảng" (e.g. "Bảng 2.1") and widen the number box to fit.
\renewcommand{\cfttabpresnum}{\bfseries Bảng\space}
...
% Prefix each entry with bold "Hình x.x"; \bfseries inside the presnum also
% bolds the number since both sit in the same \numberline box.
\renewcommand{\cftfigpresnum}{\bfseries Hình\space}
...
% float.sty lists ignore tocloft's presnum, so prefix via a group-local
% \numberline override; the fixed-width box keeps the dot leaders aligned.
{
  \renewcommand{\numberline}[1]{\makebox[7.5em][l]{\bfseries Giải thuật~#1}}%
  \listofalgorithms
}
