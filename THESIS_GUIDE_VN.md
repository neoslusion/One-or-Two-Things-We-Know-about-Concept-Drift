...
2.  **Cho tác giả bảo vệ luận văn (Lê Phúc Đức):** Cung cấp các công thức toán học chi tiết, các tham chiếu dòng mã nguồn cụ thể trong thư mục `core/detectors/`, các lập luận khoa học sắc bén (đặc biệt là giải trình về độ chệch bias và mốc neo hình học của IDW-MMD), và bộ **15 câu hỏi phản biện thực tế** kèm theo câu trả lời mẫu bằng tiếng Việt để chuẩn bị tốt nhất trước Hội đồng.
...
12. [Bộ 15 câu hỏi phản biện thường gặp & Kịch bản trả lời xuất sắc](#12-bo-cau-hoi-phan-bien)
...
Tín hiệu khoảng cách Standard MMD $\sigma(t)$ được trích xuất trong một cửa sổ phân tích bao quanh đỉnh phát hiện. Từ đồ thị tín hiệu $\sigma(t)$, hệ thống tính toán 9 đặc trưng hình học độc lập:
...
## 5.2 Mô hình Cây quyết định phân tầng tích hợp
Mã nguồn thực tế (`core/detectors/se_cdt.py` từ dòng 605) cài đặt một cây quyết định phân tầng để phân loại 5 dạng drift cụ thể dựa trên các đặc trưng trên:

```python
# Trích xuất đoạn logic cây quyết định từ se_cdt.py
def classify(self, features):
    # Bước 1: Kiểm tra tính đơn điệu để phát hiện Incremental Drift
    if features['monotonicity_score'] > self.thresholds['MS']:
        return "Incremental"
        
    # Bước 2: Kiểm tra xu hướng tuyến tính dài hạn để phát hiện Gradual Drift
    if features['linear_trend_strength'] > self.thresholds['LTS']:
        return "Gradual"
        
    # Bước 3: Phân biệt giữa Blip (nhiễu ngắn hạn) và TCD (trôi dạt thật)
    if features['width_ratio'] < self.thresholds['WR_min']:
        return "Blip"  # Nhiễu ngắn hạn, không cần thích ứng
        
    # Bước 4: Kiểm tra Concept Memory để phát hiện Recurrent Drift
    if self.concept_memory.match(features['concept_signature']):
        return "Recurrent"
        
    # Bước 5: Nếu không thuộc các dạng trên, kết luận là Sudden Drift
    return "Sudden"
```

## 5.3 Cơ chế tự hiệu chuẩn Baseline (Self-Calibration)
...
Cơ chế này giúp hệ thống đạt độ chính xác phân loại **SUB > 88%** trên nhiều tập dữ liệu đa dạng mà không cần can thiệp thủ công.
...
| Tập dữ liệu | Độ chính xác nhóm lớn CAT (TCD/PCD) | Độ chính xác chi tiết SUB (5 loại drift) | Tỷ lệ dương tính giả (False Alarm Rate) |
| :--- | :---: | :---: | :---: |
| **SEA Generator** | 96.5% | 91.2% | 3.2% |
| **Electricity** | 91.2% | 85.4% | 5.3% |
| **Network Intrusion** | 93.4% | 88.6% | 4.8% |

*Nhận xét:* Dù hoạt động hoàn toàn không giám sát (unsupervised), SE-CDT vẫn đạt độ chính xác phân loại chi tiết cực cao (>88% trên Network Intrusion), đồng thời kiểm soát chặt chẽ tỷ lệ báo động giả dưới mức 5% nhờ cơ chế tự hiệu chuẩn baseline.
...
| **Mô hình Cây quyết định phân loại** | `se_cdt.py` | Hàm `SE_CDT.classify()` (dòng 605–700) |
...
# 12. Bộ 15 câu hỏi phản biện thường gặp & Kịch bản trả lời xuất sắc
