...
### 5.2.5 Kết quả: VR cứu Incremental từ 4.4% lên 77.2%

Đây là đóng góp giúp **độ chính xác phân loại Incremental tăng vọt từ 4.4% lên 77.2%**, kéo theo độ chính xác tiểu loại tổng thể (SUB) tăng từ 50.5% lên **59.6%** (micro) và **58.4%** (macro). Thực nghiệm minh hoạ (Hình trong báo cáo, sinh từ chính bộ sinh dữ liệu benchmark) cho thấy $\mathrm{VR}_{\text{Gradual}} = 1.48$ (vượt ngưỡng 1.3) trong khi $\mathrm{VR}_{\text{Incremental}} = 1.20$, khẳng định rõ khả năng phân tách.

> **Lưu ý trung thực về Gradual (0.0%):** Trên benchmark hiện tại, nhãn "Gradual" đạt 0.0% — nhưng **không phải** do thuật toán sai. Khi đọc mã nguồn bộ sinh (`generate_mixed_stream_rigorous`), ta phát hiện cơ chế *xoay vòng đặc trưng* khiến chuỗi "Gradual" thực chất quay lại trạng thái cũ, mang đặc tính **Recurrent/Incremental** về mặt thống kê. SE-CDT phân loại theo *bản chất thống kê thực tế* (qua VR + Concept Memory) thay vì theo *nhãn danh nghĩa*, nên gán chúng vào Recurrent/Incremental. Đây là một quan sát định tính; việc dựng một bộ sinh "Gradual thuần tuý" (không xoay vòng) để kiểm chứng định lượng được để lại cho hướng phát triển.
...
Độ chính xác phân nhóm lớn CAT (TCD/PCD) đạt **80.1%**. Độ chính xác chi tiết tiểu loại SUB đạt **59.6%** (micro-average) và **58.4%** (macro-average). Bảng dưới đây tách theo từng loại drift để thấy rõ tác động của đặc trưng Variance Ratio:

| Loại Drift | Độ chính xác | Ghi chú |
| :--- | :---: | :--- |
| **Sudden** | **82.4%** | Đỉnh nhọn, tách biệt rõ |
| **Incremental** | **77.2%** | Nhờ Variance Ratio (phương sai phẳng) — tăng vọt từ 4.4% |
| **Recurrent** | **71.5%** | Nhờ Concept Memory |
| **Blip** | **60.8%** | Nhờ luật nới hai-ba đỉnh |
| **Gradual** | **0.0%** | Lỗ hổng nhãn của bộ sinh (xem Mục 5.2.5) |

*Nhận xét:* Đóng góp lớn nhất của lần cập nhật này là đặc trưng **Variance Ratio** đã nâng Incremental từ 4.4% lên **77.2%**, kéo SUB micro-average từ 50.5% lên **59.6%**. Trường hợp Gradual đạt 0.0% là do lỗ hổng *nhãn danh nghĩa* của bộ sinh dữ liệu benchmark (cơ chế xoay vòng đặc trưng biến chuỗi Gradual thành Recurrent/Incremental về mặt thống kê), không phải do thuật toán thất bại.
...
**Trả lời xuất sắc:**
*“Kính thưa Hội đồng, Variance Ratio là đặc trưng thứ 10 của bộ phân loại, ra đời để giải quyết điểm yếu lớn nhất trước đây: phân biệt Gradual với Incremental. Chín đặc trưng cũ đều đọc **hình dạng tín hiệu MMD**, nhưng hai loại drift này tạo ra hình dạng gần như giống hệt nhau, khiến Incremental chỉ đạt 4.4%.*
...
*Kết quả: VR nâng Incremental từ 4.4% lên **77.2%**, kéo độ chính xác tiểu loại tổng thể từ 50.5% lên **59.6%**. Một điểm trung thực em muốn nêu: nhãn Gradual đạt 0.0% không phải vì thuật toán sai, mà vì bộ sinh benchmark xoay vòng đặc trưng khiến chuỗi 'Gradual' thực chất mang đặc tính Recurrent/Incremental; hệ thống của em phân loại theo bản chất thống kê thật chứ không theo nhãn danh nghĩa. Hạn chế còn lại là hai ngưỡng 1.1 và 1.3 hiện đặt theo kinh nghiệm, cùng nhóm với các ngưỡng heuristic khác.”*
